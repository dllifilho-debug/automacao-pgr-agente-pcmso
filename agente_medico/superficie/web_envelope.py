...
def montar_volta_envelope(dados: dict[str, Any], validade: str, assinatura: bool) -> str:
    """Recebe os dados já carregados do artefato-ida, a validade escolhida
    (data ISO AAAA-MM-DD) e a assinatura do RT, e devolve o artefato-volta.
    Levanta ValueError se `validade` não for uma data ISO válida — mesmo
    espelho de mensagem que o prompt da CLI. Self-check final reutiliza
...
def pagina_envelope() -> None:
    ...
    st.subheader("Candidatas de validade")
    ...
    validade = st.text_input("Validade", value=(proposta if proposta is not None else ""))
