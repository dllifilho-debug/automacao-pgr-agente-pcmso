...
10. **Contagem de ocorrências tratada como lista de sítios editáveis.** `grep -c` devolve quantas vezes um literal aparece — não quais dessas ocorrências o artefato pode legitimamente mudar. Ocorrência dentro de registro histórico (bloco de sessão passada, parágrafo de tiragem anterior, redação preservada de regra `DEPRECATED`) **não é sítio editável**: reescrevê-la falsifica o que aquela sessão mediu. Para cada ocorrência que o artefato diz que vai tocar, confira se ela é estado corrente ou registro histórico. `[MEDIDO — 003.FC: o commit 6895958 reescreveu "1137 passed" dentro do registro da tiragem 003.EZ, tratando a contagem de grep -c como alvo de edição; corrigido pela Emenda 9bc875d.]`
11. **Cláusula que contradiz o corpo do próprio artefato.** Compare cada cláusula normativa contra os fatos que o próprio artefato afirma no Contexto, na motivação e nos exemplos — e, quando a cláusula classifica um estado do sistema, contra o que o repo realmente faz nesse estado. `[MEDIDO — 003.FC: a cl.2 de D-ARQ-82 classificava fuzzy_recusado como acerto do motor, enquanto o Contexto da mesma decisão nomeava o termo que sai nesse estado (Metiletilcetona) como lacuna com conduta devida por R-BIO-04 — a decisão classificava como acerto do motor o caso de dano que a justificou.]` Esta classe mora aqui, e não na barra do Crítico, por medição: o Gauntlet leu a decisão inteira a frio e não pegou a contradição, porque ela só é visível cruzando o texto com o comportamento do resolver — trabalho de quem lê o repo. Ver `D-ARQ-84` cl.5.
