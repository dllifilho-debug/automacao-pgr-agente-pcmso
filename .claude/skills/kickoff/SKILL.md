...
5. **Dívidas abertas** — varrer os DOIS docs: `docs/DECISOES_ARQUITETURAIS.md` E `docs/PROTOCOLO_AGENTE_MEDICO.md` (seção de DTs). Listar todo header `### DT-`/`### DH-` cujo marcador de status NÃO contenha FECHADA, RESOLVIDA ou REENQUADRADA (match invertido — o vocabulário de aberto varia: `[ABERTA]`, `[A VALIDAR]`, `[INCERTO]` etc.), uma linha cada, nomeando o doc de origem. A fila do HISTORICO é da última sessão e pode não citar dívida aberta antes dela. *(Correção 003.DQ: a varredura só-DECISOES reportou "1 DT aberta" quando o PROTOCOLO carregava mais de uma dúzia, incluindo as 3 que o PAINEL declara como travando produção.)*
