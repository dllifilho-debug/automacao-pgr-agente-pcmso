...
**Nota de aplicação (branch `claude/inspiring-turing-0ylkmk`, 24/09/2026 — DT-003EC-01).** A distinção que esta decisão nomeou ("sem medição" ≠ "apenas qualitativa") ganha produtor real pela primeira vez: `apenas_qualitativa` estava no tipo desde 002.L0, mas nenhum extrator o escrevia para sílica. Agora `_helper_silica_asbesto` o preenche no ramo `q is None` quando o risco é sílica e o PGR declara nível P×S na linha (`Risco.nivel_risco`, vindo de `RiscoVerbatim.avaliacao_qualitativa`), e `silica_asbesto_sem_medicao`/`silica_qualitativa` partem o ramo sem-avaliação por esse campo — disjuntos por construção. Conduta nova do lado qualitativo: 12M (`R-RX-01-qual`, `[INTERPRETADO]`, decisão do Diovanni), e não a leitura da v2 ("qualitativa → 12M" como se fosse o ramo da norma): o ramo normativo segue 24M e o 12M vem declarado como precedente mais protetivo. Nenhuma cláusula alterada; `periodicidade_apos_15a` segue a mesma semântica (`None` no 12M constante).
...
**Reabertura parcial (sessão atual, branch `claude/vigilant-faraday-d8a4wn`, número não atribuído) — só o Decreto 3.048, só dado.** Diovanni autorizou reabrir a fatia previdenciária de `DT-003BA-01` antes do cutover — decisão dele, não medição que refuta a original; a redação acima ("resolver NO cutover") permanece exata para eSocial (Tab 24) e demais saídas de engenharia, que seguem FORA e não fazem parte desta reabertura. Escopo estrito: campo `enquadramento_3048` em `agentes.yaml` (D-ARQ-12), dado puro, **sem consumidor no motor** — nenhum campo novo em `Risco`, nenhuma emissão, nenhuma `R-*` tocada. Fonte: Decreto 3.048/1999, Anexo IV ("Classificação dos Agentes Nocivos"), texto compilado oficial `planalto.gov.br/ccivil_03/decreto/d3048compilado.htm`, obtido do Diovanni (egresso deste domínio bloqueado pela política de rede da sessão) e conferido item a item contra os 27 slugs do vocabulário que citam agente nomeado no Anexo IV.

**Achado — o `dec_3048` do motor legado (`modules/modulo_engenharia.py::DICIONARIO_CAS`) errava em pelo menos 2 entradas**, mesma classe de erro já registrada para `agente_medico_ia.py` (hardcode sem verificação): tolueno e xileno eram gravados como item "1.0.19, 25 anos" — o item 1.0.19 (Grupo I) cita literalmente "diisocianato de tolueno (TDI)", não tolueno nem xileno puros. O motor novo grava `null` para os dois e cria slug próprio `tdi` → 1.0.19, corrigindo a confusão substância-base/composto. `dissulfeto_de_carbono` era rotulado "Não Enquadrado" pelo legado; consta literalmente no item 1.0.11 — corrigido para `{codigo: "1.0.11", tempo_exposicao: "25 anos"}`. `vibracao_mao_braco`/`vibracao_corpo_inteiro` recebem o mesmo item único "2.0.2 Vibrações" do Decreto — o legado tratava como dois itens com limiar numérico (m/s² da NR-15); o texto oficial não distingue localização nem cita limiar, só a atividade "perfuratrizes e marteletes pneumáticos" — ressalva registrada no comentário do YAML.
...
**Aplicação na sessão atual (branch `claude/nice-ptolemy-wxk1wo`, IMPLEMENTAÇÃO, pedido do Diovanni) — `preparar_composicao` ganha o 1º chamador de produção: upload avulso de FDS em `pagina_matriz`, sem casamento com produto do PGR (fatia 1 de 2).** Até esta sessão `preparar_composicao` (`orquestracao_fds.py`) só era chamada em teste — nenhuma superfície de produção invocava a cascata extração→transcrição-LLM→gate de forma (nota da sessão 003.BF acima). `pagina_matriz()` (`web_matriz.py`) ganha um `st.file_uploader` de FDS/FISPQ próprio, com `accept_multiple_files=True`, posicionado ANTES do `if arquivo is None: return` do PGR — funciona com ou sem PGR enviado (pedido explícito do Diovanni: "e se eu quiser somente extrair da FDS sem o PGR"). Para cada FDS enviada: grava em tempfile, chama `preparar_composicao(caminho, TranscritorGemini())` (cliente real, mesmo adaptador da sessão 003.BG), renderiza CAS/nome/frases-H por membro + pendências. Cl.4 de D-ARQ-47 preservada por construção: esta fatia só expõe o que `preparar_composicao` já fazia (para em `gate_forma`) numa tela nova — não compõe `montar_fds`/`resolver_composicao`, não é bypass de revisão-RT. Escopo desta fatia é deliberadamente só a extração avulsa: **falta o casamento entre a FDS enviada e o `ProdutoQuimico.nome` que o PGR declara** (peça que conecta o CAS/frases-H ao GHE/cargo certo e alimenta `resolver_composicao`/`materialidade`, fatia 2) — decisão já tomada com o Diovanni: seleção MANUAL na tela (lista de produtos que a IA leu do PGR), não casamento automático por nome, para não arriscar vínculo errado e silencioso (classe D-ARQ-22). `DT-(sessão claude/nice-ptolemy-wxk1wo)-01` aberta em `PENDENCIAS_CLINICAS.md` para a fatia 2. Testado ao vivo: `streamlit run app_matriz_local.py` + upload real de um FISPQ extraído do PGR Hetrin/set-2026 (`matrizes_originais/`) — extração real localizou a seção de composição (não caiu em `composicao_ausente_fds`) e a tentativa de transcrição sem `CHAVE_API_GOOGLE` no ambiente virou `transcricao_indisponivel_fds` na tela, sem exceção — confirma a costura ida completa até o ponto em que falta credencial. 4 testes novos (`test_web_matriz.py`, AppTest): FDS sem PGR mostra composição; sem upload de FDS não chama `preparar_composicao`; pendência da FDS aparece na tela; upload de FDS não interfere no fluxo normal do PGR — varredura inversa confirmada (`git stash` do código-fonte, mantendo os testes, derruba os 4 e só os 4). `mypy --strict` alvo canônico limpo, 49 arquivos, delta-zero. Nenhuma `R-*` criada/alterada; PROTOCOLO inalterado. `[IMPLEMENTADO — sessão claude/nice-ptolemy-wxk1wo; consome D-ARQ-47 cl.1-4]`
...
[NOTA (branch `claude/inspiring-turing-0ylkmk`, 24/09/2026) — `RiscoVerbatim` ganha o campo aditivo `avaliacao_qualitativa: str = ""` (banda S·P·NÍVEL da matriz P×S, verbatim), preenchido só pela rota determinística (`parser_familia_consciente`, banda calibrada por bloco no cabeçalho, molde AGENTE/FONTE); rotas LLM e card deixam `""`. Mesma semântica cravada da P3: texto cru, parse resolver-side (`hidratacao.parsear_nivel_risco` → `RiscoPGR.nivel_risco`). Não reintroduz o grid I/O/T/EP/PE/E descartado pela P3-C3 — é a coluna de nível de risco, outra grandeza. Consumidor: `R-RX-01-qual` (DT-003EC-01); insumo reservado para `DT-003EB-02`. Nenhuma cláusula alterada.]

[NOTA (branch `claude/eager-fermat-txbn7h`, 24/09/2026) — a rota LLM passa a preencher
`avaliacao_qualitativa`: regra 6b nos dois prompts de `transcritor_gemini_pgr` (S·P·NÍVEL verbatim,
só na escala P×S) e exceção explícita na regra 7. Toca a C3 de `D-ARQ-50` só nisto: a coluna de
nível P×S deixa de ser ruído; o grid I/O/T/EP/PE/E e o escore somado continuam ruído. Guarda
determinística em `transcrever_ghes` (`_restringir_avaliacao_a_escala_pxs`): nível só sobrevive
em bloco com a legenda "Irrelevante" — medido 109/109 blocos aceitos na escala P×S e 0/318 na de
escore, onde o padrão S·P·NÍVEL casaria falsos níveis. Nenhuma cláusula alterada.]
...
**Consequência observada (sessão atual, branch `claude/nice-ptolemy-wxk1wo`) — o adiamento de "FDS apontadas" (Parte 2) segue vivo em produção, e agora tem consumidor real esperando.** Ao planejar a fatia 2 do plug FDS→produção (`DT-(sessão claude/nice-ptolemy-wxk1wo)-01`, `PENDENCIAS_CLINICAS.md`), medido: `hidratar_pgr` (`motor/hidratacao.py:137`) crava `GHEPGR.produtos_quimicos=()` sempre — nenhuma extração de nome de produto químico por GHE existe hoje, exatamente o campo que esta decisão nomeou como diferido na Parte 2 e a nota 003.BN não retomou na forma concreta implementada (`GHEVerbatim`/`RiscoVerbatim` não têm campo de apontador-FDS distinto). Consequência prática: `resolver_composicao`/`executar_com_composicao` (D-ARQ-40, já rodam dentro de `processar_pgr`) nunca têm o que resolver em produção — a cadeia química fica sem insumo, não por bug, por adiamento nunca revisitado. Diovanni decidiu não implementar essa extração agora (fora do tamanho da fatia 2) nem usar `RiscoVerbatim.agente`/`fonte_geradora` como substituto não-medido — parou a sessão. Nenhuma cláusula alterada; a decisão de adiar continua vigente, só ganhou consumidor nomeado. `[MEDIDO — leitura de `motor/hidratacao.py`/`motor/tipos.py`, sessão claude/nice-ptolemy-wxk1wo]`

**Medição do substituto (mesma sessão, mesmo dia) — descartado por dado real; achado novo para quem for retomar a Parte 2.** `parsear_arquivo` (`parser_familia_consciente.py`, D-ARQ-65, determinístico) contra o PGR real Consciente/Fascino: `agente`/`fonte_geradora` não têm granularidade de produto — `fonte_geradora="Exposição a tintas e seus componentes."` sozinha agrupa ~15 `agente` distintos (componentes químicos individuais da tabela de composição de 1 produto, já decompostos no texto do PGR). É 1 FDS : N linhas de `agente`, nunca 1:1 — inverso do N:1 medido na fatia 5b de `D-ARQ-57` mais cedo nesta sessão. Quem retomar a extração formal (Parte 2) precisa decidir a unidade de casamento como **grupo por `fonte_geradora`**, não "produto nomeado" — o PGR (ao menos este witness) não nomeia produto, só declara componentes agrupados por atividade/contexto de exposição. `[MEDIDO — parsear_arquivo real contra o Fascino, sessão claude/nice-ptolemy-wxk1wo]`

**ARQUITETURA PROPOSTA (sessão atual, branch `claude/sharp-wozniak-j4596a`) — o agrupamento por `fonte_geradora` sozinho não basta; achado novo derruba a extração automática, produtos_quimicos passa a ser CRIADO PELO RT, não extraído na hidratação.** Retoma `DT-(sessão claude/nice-ptolemy-wxk1wo)-01` pelo único caminho nomeado (b). Gate de estado real: `hidratar_ghe` (`motor/hidratacao.py:131-140`) crava `produtos_quimicos=()`; `ProdutoQuimico(nome: str, fds: Optional[FDS])` (`motor/tipos.py:180-182`) já existe, sem consumidor de produção; `resolver_composicao` (`motor/composicao.py:82-100`) e a Fase C de promoção (`estagios/riscos.py:112-152`) já iteram `ghe.produtos_quimicos` tratando cada produto independente de qualquer `RiscoPGR` — a granularidade de "produto" não precisa casar 1:1 com `RiscoVerbatim.agente`.

*Achado que muda a decisão — `stage_3_pendencias_estruturais`/R-PGR-04 (`estagios/pendencias_estruturais.py:7-19`; `PROTOCOLO_AGENTE_MEDICO.md:65-66`, `[VALIDADO]`) emite Pendencia BLOQUEANTE `composicao_ausente` para TODO `produto` com `fds is None`.* Essa regra nunca disparou em produção porque `produtos_quimicos` sempre foi `()`. Populá-lo automaticamente a partir de QUALQUER `fonte_geradora` não-vazia dispararia R-PGR-04 para grupos que não são produto químico algum. Medição reproduzível — `parsear_arquivo` (`parser_familia_consciente.py`, já versionada, determinística, sem LLM) sobre `matrizes_originais/PGR - CONSCIENTE CONSTRUTORA E INCORPORADORA SPE 0030 - FASCINO  (15.07.26).pdf`, agrupando `ghe.riscos` por `.fonte_geradora` (qualquer sessão futura reproduz com as mesmas 2 linhas de código, sem script à parte): 19 GHEs, 237 riscos. Grupos com `n>1` medidos: FÍSICOS `fonte='Operação de máquinas e equipamentos eletroportáteis...'` → `n=2`, `['Ruido', 'Vibrações localizadas (mão e braço)']` (repetido em 5 GHEs distintos) e `fonte='Poeiras geradas no processo produtivo...'` → `n=2`, `['Sílica livre', 'Poeira respirável']` (2 GHEs); QUÍMICOS `'Exposição a cimento e concreto'` → `n=10` e `'Na execução do trabalho de encanação'` → `n=8` (a tinta da medição da v198, noutro GHE do mesmo documento, `n≈15`). **Neste witness há uma lacuna limpa (físico ≤2, químico ≥8) — mas um único witness não prova invariante.** Nada na estrutura do PGR impede uma 3ª fonte física sob a mesma descrição (ex. "operação de máquina a céu aberto" combinando Ruído+Vibração+Radiação não-ionizante, `n=3`, as três já presentes como riscos físicos SEPARADOS em outros GHEs deste mesmo documento) nem um produto de 2 componentes (`n=2`) — o padrão observado é do documento medido, não da forma do PGR. Cravar um corte de `n` sem essa garantia estrutural inventa categoria sem fonte normativa (D-ARQ-22) — o mesmo defeito, por outra via, do corte por vocabulário: `agentes.yaml` não tem campo de categoria química/física utilizável como filtro (`cas`/`tipo_ibe`/`is_carcinogeno_iarc`/`is_ototoxico`/`tem_lt` — nenhum é proxy confiável; `cas: null` é a MAIORIA dos slugs, DT-003M-02(A)), e o texto original de D-ARQ-49 P3-C3 já descartou como ruído o único campo do PGR que discriminaria por desenho (grid de classificação I/O/T/EP/PE/E) — reintroduzi-lo é medição/decisão própria, fora desta sessão. Extração automática por `fonte_geradora`, sem filtro com garantia estrutural, arrisca pendência bloqueante falsa em GHEs físicos de PGRs reais que este witness não mostrou — regressão de produção, não avanço. `[MEDIDO — parsear_arquivo real contra o Fascino, sessão claude/sharp-wozniak-j4596a; reproduzível a partir da função já versionada, sem script externo]`

*Decisão — produtos_quimicos nasce vazio na hidratação (sem mudança em `hidratar_ghe`/`hidratar_pgr`); o RT o povoa explicitamente na tela, sempre com `fds` já anexada.* Nenhuma extração automática de "produto químico candidato" a partir de `fonte_geradora`/`agente`/qualquer heurística de contagem. Em vez disso, a fatia de UI (abaixo) deixa o RT, ao enviar uma FDS avulsa com um PGR já carregado, escolher a QUAL GHE ela se aplica e dar um nome ao produto (texto livre, ou pré-preenchido a partir de uma `fonte_geradora` mostrada como referência — decisão de tela, não desta ARQUITETURA) — o ato de confirmar CRIA o `ProdutoQuimico(nome=..., fds=fds_extraida)` diretamente com `fds` populado, nunca `fds=None` órfão. Consequência: R-PGR-04 nunca dispara por esta via (o slot só nasce já resolvido) — a regra continua com seu gatilho original intacto (PGR que aponta produto sem dar composição), que segue não-implementado e não é desta fatia; nenhuma regressão, nenhum avanço nesse ramo específico, escopo negativo explícito abaixo. Universalidade — responde sim aos três domínios, mas não com custo igual: o mecanismo não depende de heurística sobre o texto do PGR (construção civil/Fascino, saúde com fonte_geradora tipicamente biológica e químico minoritário, química com produto nomeado explícito passam pelo mesmo caminho), porque não INFERE nada do documento, só reage à escolha do RT — mas o custo operacional é digitação humana proporcional ao nº de produtos por GHE, e **numa planta química intensiva (muitos produtos por GHE) esse custo não foi medido contra um witness real** — paliativo sinalizado, não escondido: favorece correção (zero pendência falsa, zero vínculo errado) sobre automação, às custas de trabalho manual do RT; se esse custo se provar proibitivo em uso real, a retomada é medição própria contra um PGR de indústria química (fora desta sessão, sem esse witness em mãos).

*Fatia 2a — split cheap/expensive no adaptador.* `processar_arquivo_pgr` (`orquestracao_pgr.py:232-287`) encadeia `preparar_ghes` (caro: I/O+LLM) → `hidratar_pgr` (barato, puro) → `processar_pgr` (barato, puro — já roda `resolver_composicao` via `executar_com_composicao`, `motor/entrada.py:13-15` + `motor/orquestrador.py:197`) numa chamada só, descartando o `PGR` hidratado intermediário. Extrair uma função (nome à escolha do Code) que devolve `tuple[PGR | None, tuple[Pendencia, ...]]` parando logo após `hidratar_pgr`; `processar_arquivo_pgr` vira wrapper fino sobre ela + `processar_pgr`, MESMO comportamento externo — a suíte existente de `processar_arquivo_pgr` (verde sem alteração de asserção) é o discriminante do refactor. Fronteira: só `orquestracao_pgr.py` + teste novo/existente; motor (`entrada.py`/`orquestrador.py`) intocado.

*Fatia 2b — UI: o RT cria o produto e anexa a FDS.* `pagina_matriz` (`web_matriz.py:243-426`) passa a cachear o `PGR` hidratado (campo novo em `CacheMatrizes`, ao lado de `matrizes`/`status`— não entra em `calcular_chave_cache`, é resultado do parse, não identidade dele, molde `chamadas_ia`). Para cada FDS avulsa enviada (loop já existe, `web_matriz.py:273-286`) com PGR carregado, a tela oferece escolher o GHE (`st.selectbox` sobre `pgr.ghes`) e um nome (texto livre) para o produto; ao confirmar, `dataclasses.replace` cria o `ProdutoQuimico` já com `fds` populada e o anexa ao GHE escolhido (`produtos_quimicos=(*ghe.produtos_quimicos, produto_novo)`, replace aninhado GHE→PGR), e a tela roda `processar_pgr(pgr_atualizado, protocolo, hoje)` de novo — SEM reprocessar o PDF, SEM nova chamada LLM. Sem PGR carregado, comportamento idêntico ao de hoje (fluxo avulso puro). Discriminante: suíte existente de `web_matriz`/`orquestracao_pgr` permanece verde (refactor não muda saída); teste novo — FDS anexada a 1 produto atravessa até `Resultado.matrizes` com o componente resolvido/promovido (Fase C), com contador de chamadas LLM (`_TranscritorContado`) em zero no rerun. Reversão que mata: não persistir o `PGR` mutado entre reruns do Streamlit — o componente some no rerun seguinte. Fronteira: `orquestracao_pgr.py` (só se a fatia 2a não bastar) + `web_matriz.py` + testes correspondentes; motor intocado.

*Escopo negativo.* NÃO extrai produto/composição automaticamente de `fonte_geradora`, `agente` ou qualquer heurística de contagem — descartado por medição nesta sessão. NÃO filtra quimico/físico por `agentes.yaml` — não existe campo confiável. NÃO reintroduz o grid de classificação I/O/T/EP/PE/E descartado por D-ARQ-49 P3-C3. NÃO implementa o gatilho original de R-PGR-04 (produto apontado sem FDS vira pendência) — seguiria não-implementado após esta fatia, honestamente, não escondido. Nenhuma R-* criada/alterada/depreciada; nenhum campo novo em `tipos.py`.

**Status:** ARQUITETURA PROPOSTA. Aguarda ratificação do Diovanni antes de virar fila de IMPLEMENTAÇÃO (molde D-ARQ-57 peça 5, v189→v192). Sequenciamento: 2a antes de 2b (`INSTRUCOES_ARQUITETO.md` §7, uma fatia por sessão do Code); se o Code medir que 2b excede 1 fatia, o split fica a critério dele, reportado. `DT-(sessão claude/nice-ptolemy-wxk1wo)-01` recebe nota apontando esta proposta. `[DERIVADO — leitura de motor/tipos.py, motor/hidratacao.py, motor/composicao.py, motor/estagios/riscos.py, motor/estagios/pendencias_estruturais.py, motor/entrada.py, motor/orquestrador.py, adaptadores/orquestracao_pgr.py, superficie/web_matriz.py, protocolo/vocabulario/agentes.yaml, PROTOCOLO_AGENTE_MEDICO.md §R-PGR-04; medição ao vivo com parsear_arquivo contra o Fascino real; sessão claude/sharp-wozniak-j4596a]`

**Ratificação (mesma sessão, branch `claude/sharp-wozniak-j4596a`, 20/09/2026).** Diovanni ratifica a decisão acima como desenhada — produtos_quimicos criado pelo RT na tela, não extraído automaticamente por `fonte_geradora`; fatiamento 2a→2b mantido, sem reordenar. O gap nomeado pelo Crítico na 2ª rodada (universalidade em indústria química não medida contra witness real; custo de digitação manual por produto/GHE pode pesar) fica aceito, não mitigado — mesma classe de decisão explícita, não omissão, do molde D-ARQ-57 peça 5 (v189). D-ARQ-49 Parte 2 passa de ARQUITETURA proposta para **ARQUITETURA ratificada**, liberada para IMPL a partir da fatia 2a. Sem código nesta sessão; nenhuma cláusula de D-ARQ alterada, nenhuma D-ARQ nova. `[RATIFICADO — Diovanni; sessão branch claude/sharp-wozniak-j4596a]`

**Andamento (sessão atual, branch `claude/blissful-johnson-mdeqn5`) — fatia 2a IMPLEMENTADA.** `preparar_pgr_hidratado` extraída em `orquestracao_pgr.py` (`preparar_ghes` -> `hidratar_pgr`, parando ANTES de `processar_pgr`), devolvendo `tuple[PGR | None, tuple[Pendencia, ...]]` como especificado (v199); `processar_arquivo_pgr` vira wrapper fino sobre ela + `processar_pgr` — MESMO comportamento externo, nenhuma assinatura pública muda, nenhum arquivo fora de `orquestracao_pgr.py` tocado (motor intocado, fronteira respeitada). Discriminante: a suíte EXISTENTE de `processar_arquivo_pgr` é o único critério nomeado na ARQUITETURA para este refactor — nenhum teste novo pedido, nenhum entra. Recorte que cobre os derivados tocados (`test_orquestracao_pgr.py`+`test_web_matriz.py`+`test_documento_matriz.py`): **64 passed**, verde sem alteração de asserção. Suíte completa (`agente_medico/tests/ tests/`, árvore parada): **1278 passed, 6 skipped, 0 failed** — delta-zero exato contra o Baseline herdado (v200, `main 3d1b33b`). `mypy --strict` alvo canônico limpo, **49 arquivos**, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. Fatia 2b (UI em `web_matriz.py` — RT escolhe o GHE e nomeia o produto ao anexar a FDS) segue aberta, próxima da sequência 2a→2b.
...
**Nota (mesma sessão, mesmo dia) — medição aprofundada: "aceitar cabeçalho CAIXA ALTA" tem escopo maior do que uma checagem pontual; decisão do Diovanni é não perseguir agora.** A pedido do Diovanni, medi o que faltaria pra de fato alcançar o cabeçalho de Hetrin/set-2026, e o gap é maior do que title-case vs. CAIXA ALTA:

1. **Banda Função muito mais estreita** (~50-83pt, contra ~60pt calibrado nos 2 witnesses limpos) — cada nome quebra em quase 1 palavra por linha física. Medido o único caso N:1 real do documento (pág. 14, 0-indexed, top 373.9-452.4pt, banda `x0` 50.5-63.6): `'ENGENHEIRO'` `'CIVIL/'` `'ENGENHEIRO'` `'RESIDENTE/'` `'ESTAGIÁRIO'` `'DE'` `'ENGENHARIA/'` `'APONTADOR'` `'ADMINISTRATIVO'` `'DE'` `'OBRA/'` `'TÉCNICO'` `'DE'` `'SEGURANÇA'` `'DO'` `'TRABALHO'` — 6 cargos (bate com a leitura qualitativa já registrada na nota de ARQUITETURA original), mas **falta o `/` entre `'APONTADOR'` e `'ADMINISTRATIVO'`** no texto extraído — decompor só por `/` funde os dois num cargo só ("Apontador Administrativo de Obra"), divergindo da matriz já aprovada desse documento (6 cargos distintos, achado da sessão `claude/dreamy-mayer-os6jce`). Não determinado se a barra falta no PDF-fonte ou se é perda de extração — não investigado além deste ponto.
2. **O cabeçalho não repete em toda página do intervalo.** Grid medido nas págs. 14-36 (0-indexed) por presença da palavra `'FUNÇÃO'`/`'FUNÇAO'` — todas as páginas do intervalo têm, EXCETO a pág. 32 (conteúdo de corpo sem nenhum cabeçalho de coluna, aparência de continuação sem repetição). O contrato da fatia 5a ("recalibra por página; página sem cabeçalho reconhecível é `GrupoFuncaoNaoReconhecido`") quebra para esse witness sem mudança de desenho — precisaria herdar a calibração da última página com cabeçalho, contrato distinto do medido nos 2 witnesses limpos (que sempre repetem).

**Ponto que decidiu a sessão — mesmo com a 5b completa para este witness, o documento não ingere hoje.** Faltam as fatias 5c (parsing da célula de risco) e 5d (roteamento + plug em `preparar_ghes`), nenhuma das duas implementada. Sem elas, Hetrin/set-2026 já cai em `pgr_cargo_based` bloqueante e vai a revisão humana — gated by design, mesma classe de HUMAP/Cjr (`DT-003DK-01`), sem risco de dado errado. Investir na calibração própria desse layout (banda estreita + herança de cabeçalho + regra para o separador ausente) agora adianta trabalho para um pipeline que ainda não fecha, por 1 documento cujo status — template novo do cliente Ricco a partir de 14/09/26, ou formatação pontual desse envio — não está confirmado.

**Decisão do Diovanni (Arquiteto e usuário desta sessão):** não perseguir a calibração do Hetrin/set-2026 agora. `DT-(sessão claude/youthful-lamport-3kfkog)-02` segue ABERTA, fatiamento 5a→5b→5c→5d inalterado — só o início efetivo da 5b fica sem data, revisitado quando houver mais sinal sobre se esse layout é padrão recorrente. Nenhuma cláusula de D-ARQ alterada, nenhuma D-ARQ nova; nenhuma `R-*` tocada; PROTOCOLO inalterado. `[MEDIDO — pdfplumber host, sessão claude/nice-ptolemy-wxk1wo, contra Hetrin/set-2026; decisão do Diovanni, sem código de produção]`
...
**Andamento (sessão branch `claude/hopeful-newton-yjv3k7`, número não atribuído, 23/09/2026) — forma 6 do repertório GHE (separador antes do número) + separação de cargos por código CBO; fecha `DT-(sessão claude/hopeful-newton-yjv3k7)-01`.** Medição de `DT-003FG-01` (PR #358) achou dois PGRs que atravessavam a rota determinística perdendo GHE **sem pendência**: Porto Araras I (`GHE - 14 PINTURA`, 1 GHE) e Vila Brasil Escritório (`GHE\x00 01 \x00 ADMINISTRAÇÃO 01` … `GHE\x00 22 …`, 22 de 26 GHEs). As duas rotas usam `eh_cabecalho_ghe`, então a checagem `len(candidatos) == len(blocos)` concordava na contagem errada. Forma nova `_reconhece_cabecalho_ghe_separador_antes_do_numero` (`GHE\s*[-\x00]\s*\d+(?:\s*[-\x00]\s*|\s+)\S.*`, título obrigatório), separada da forma padrão para não aceitar `"GHE 22 SECONC"`; varredura dos 43 PDFs de `matrizes_originais/`: só casa os 23 cabeçalhos-alvo nos PGRs. Título extraído por padrão espelho em `parser_familia_consciente._extrair_titulo_ancora`. **Achado de mesma sessão, sob `D-ARQ-65` cl.5 / 003.EP fatia 2:** `_separar_nome_cbo` tratava a corrida final de `[\d\x00\s-]+` como cauda CBO — espaço puro casava, e cargo SEM CBO perdia a última palavra (Porto Araras: 26/51 cargos, "Operador de Betoneira" → "Operador de"); o Fascino não expunha o defeito porque tem CBO em 41/41. Substituído por `_PADRAO_CBO` (família de 4–5 dígitos + ocupação de 2), que também separa entradas quando a vírgula falta entre dois cargos (Vila Brasil: "… SR l \x001423\x0030\x00 Coordenador de Marketing"); trecho sem letra não vira cargo. Resultado medido: Porto Araras 16 GHEs / 52 cargos, Vila Brasil 26 GHEs / 86 cargos — **as duas contagens batem com os gabaritos pareados**, nomes divergentes só por grafia humana; Fascino idêntico (19 GHEs, 41 cargos, mesmos nomes). Gate de número de GHE saltado **medido, não implementado** (0 disparo novo no acervo pós-fix; decisão pendente em `PENDENCIAS_CLINICAS.md`). Nenhuma `R-*` tocada.

**Andamento (mesma branch `claude/hopeful-newton-yjv3k7`, pós-merge do PR #359, 23/09/2026) — gate de número de GHE saltado.** `avaliar_numeracao_ghe(paginas)` em `motor/extracao_pgr.py`, função pura, encadeada **depois** de `avaliar_segmentacao` no ramo 1 de `avaliar_estrutura` (`avaliar_segmentacao(...) or avaliar_numeracao_ghe(...)`); `avaliar_segmentacao` e `_avaliar_spans` (reusado pela rota card) intocados. Regra: todo número de 1 ao maior número reconhecido por `eh_cabecalho_ghe` aparece em algum cabeçalho; cabeçalho repetido não é defeito. Falha → `Pendencia(tipo="numeracao_ghe_lacunar", bloqueante=True, regra_origem="D-ARQ-57")` com os números ausentes no motivo. Motivo: quando o reconhecedor falha numa forma nova, as duas rotas concordam na contagem errada e o GHE some sem pendência (Porto Araras I, Vila Brasil Escritório, andamento anterior). Medido nos 29 PGRs do acervo: lacuna só em R78 e Floramazônia, já barrados antes por `segmentacao_implausivel` — **0 desfecho de `avaliar_estrutura` muda**. Adendo legítimo que traga só GHEs altos também bloqueia: falso-positivo aceito por desenho (revisão humana, anti-supressão vence), mesma escolha dos limiares da peça 2. Nenhuma `R-*` tocada.
...
`mencao_documental: {regra, niveis_risco}` nas 42 `R-BIO-04-*` do Quadro 1 (hoje só
`[IRRELEVANTE]`), lida por um desvio
...
indicador foi dispensado por risco irrelevante no PGR (`MatrizGHE.observacoes`, produzida pelo
motor). Forma copiada da anotação da Dra. Carolini no gabarito Fascino ("Incluir no word do PCMSO,
risco baixo no PGR para …"). `montar_documento` continua sem lógica de domínio — só formata o que a
...
**Nota de aplicação (branch `claude/hopeful-newton-yjv3k7`, 23/09/2026) — o remédio da emenda aplicado ao caso-âncora.** `Metiletilcetona` (grafia nua) ganhou alias em `termos:` de `metil_etil_cetona`, junto com `Adesivo CPVC Metiletilcetona` — exatamente o remédio que a emenda prescreve ("admitir a grafia em `termos:` sob `D-ARQ-70`"), dentro de `DT-(sessão claude/hopeful-newton-yjv3k7)-02` (aliases por termo, decisão do Diovanni). A grafia aparece em 3 PGRs do acervo (Fascino, Porto Araras I, Vila Brasil Escritório). Consequência no teste T7 (`test_orquestrador.py`): a testemunha real passou de `Metiletilcetona` para `Maganês` (grafia do Fascino, `fuzzy_recusado`, distância 1 de `manganes`); a checagem e a reversão nomeada (devolver `fuzzy_recusado` a `CAUSAS_ACERTO_NAO_RESOLUCAO`) ficam iguais — reversão aplicada e medida, derruba T6 e T7. Efeito medido no Fascino: **+1 linha** (`mek_urina` 6M no GHE PINTURA), nenhum status muda; o gabarito do Fascino não pede MEK para o pintor (anotação de risco baixo) — caso de `DT-003EB-02`, não desta decisão. Nenhuma cláusula alterada.
...
**Conferência normativa (25/09/2026, pós-merge do PR #382) — fecha o `[A CONFERIR]` do Contexto.** Conferido nos PDFs oficiais que o Diovanni versionou em `normas/`:

- **NR-09** (`normas/nr-09-atualizada-2026 .pdf`, sha256 `9cc608eb…a8d584`; alterações até a Portaria MTE 105/2026), item **9.6.1**, disposição transitória *"enquanto não forem estabelecidos os Anexos a esta Norma"*: **"b) como nível de ação para agentes químicos, a metade dos limites de tolerância"**; "c) como nível de ação para o agente físico ruído, a metade da dose". Item **9.6.1.1**: na ausência de LT na NR-15, usar a ACGIH. A NR-09 tem Anexo I (vibração) e Anexo III (calor), nenhum de agente químico — o 9.6.1 "b" rege a cl.5. `[DERIVADO]`
- **NR-07** (`normas/nr-07-atualizada-2022-1 .pdf`, sha256 `f27b63bb…be938b63`, o mesmo PDF da conferência de `claude/eager-fermat-txbn7h`; alterações até a Portaria MTP 567/2022): item 7.5.12 "b" confere com o texto citado no Contexto.
- **NR-15 Anexo 11** (`normas/nr-15-anexo-11 (3).pdf`, sha256 `6ee8a26b…f549bd`), Quadro n.º 1, até 48 h/semana: acetona 780 ppm, acetato de etila 310 ppm, metil etil cetona 155 ppm, tetrahidrofurano 156 ppm, tolueno 78 ppm, xileno 78 ppm, estireno 78 ppm, etilbenzeno 78 ppm, ciclohexanol 40 ppm — iguais aos de `docs/referencia/GABARITO_003DP_anexo11-12_iarc.md`. Ciclohexanona e benzeno ausentes do Quadro (benzeno "Excluído pela Portaria n.º 03/1994") — confirmam as fronteiras da cl.5 e da Q4.

Efeito lateral registrado, não aplicado: o 9.6.1 "c" dá literal ao nível de ação do ruído (metade da dose), que o docstring de `classificacao_ruido.py` chama de "item literal da NR-09 incerto". Com o critério de dose da NR-15 Anexo 1 (q=5), metade da dose equivale a 80 dB(A), o corte usado hoje.

**Nota de aplicação — fatia 1 IMPLEMENTADA (25/09/2026, pós-merge do PR #383).** Mesma ID, nenhuma cláusula alterada.

- **Dado (cl.5):** `lt_nr15: {ppm, mg_m3, fonte}` em 28 agentes de `agentes.yaml` — os da família R-BIO-04 que constam do Quadro n.º 1 do Anexo 11 (conferidos linha a linha no PDF de `normas/`); os outros 14 não têm LT ali (arsênio, n-hexano, cromo hexavalente, cobalto, nitrobenzeno, indutores de metahemoglobina, HDI, acetato de 2-metoxietila, chumbo tetraetila, ciclohexanona, furfural, MBK, MIBK, NMP).
- **Decisão clínica nova, fora do texto ratificado (Diovanni, 25/09/2026):** os 7 cancerígenos IARC 1/2A com LT (tricloroetileno, 1,3-butadieno, óxido de etileno, diclorometano, estireno, dimetilformamida, tetracloroetileno) ficam **sem** `niveis_com_medicao_abaixo_acao` — recebem o indicador com qualquer medição. A chave está em 21 regras. Achado de passagem: `is_carcinogeno_iarc` em `agentes.yaml` diverge do gabarito 003.DP para esses agentes (ex.: tricloroetileno `false`, IARC 1), por isso a exclusão foi feita por regra e não derivada do campo.
- **Código:** `ProcedenciaMedicao`, `Quantificacao.procedencia`, `MedicaoInformada`, `Observacao.medicao` (`tipos.py`); `limite_quimico`/`avaliar_medicao_quimica`/`NIVEL_ACAO_PCT_LT` (`leo_resolver.py`); `_dispensa_por_medicao` (`estagios/emissao.py`); `aplicar_medicoes` com `medicao_divergente`/`medicao_sem_risco` (`motor/medicoes.py`, novo); validação de `lt_nr15` e da chave nova (`protocolo.py`); `CacheMatrizes.medicoes`, `registrar_medicao_e_reprocessar`, `remover_medicao_e_reprocessar`, `agentes_mensuraveis` e o painel "Avaliações quantitativas" (`web_matriz.py`); célula e revisão com a medição (`documento_matriz.py`, `apresentacao_matriz.py`).
- **Interpretação registrada (cl.2):** risco sem nível P×S só é coberto pela medição quando vem de composição de FDS anexada; risco implícito (cargo) sem nível continua emitindo.
- **Medido:** 3 pares determinísticos (Fascino, Porto Araras I, Vila Brasil) com saída idêntica à `main` — nenhum traz medição de químico. Efeito real depende de laudo informado na tela `[A MEDIR — primeiro uso em produção]`.

**Nota de aplicação — fatia 2 IMPLEMENTADA (25/09/2026, pós-merge do PR #384).** Mesma ID, nenhuma cláusula alterada.

- **Escopo reduzido por medição, decisão do Diovanni:** sílica e PNOS; **asbesto fora**. O LEO-resolver não tem o asbesto (só sílica e PNOS em `_PRECEDENCIA`) e o helper de R-RX-01 exige %quartzo para calcular — incluir o asbesto mexeria no núcleo de R-RX-01. Asbesto aparece em 0 dos 29 PGRs do acervo como agente de GHE (só em texto padrão). Registrado em `DT-(sessão claude/hopeful-ramanujan-rbgh4s)-01`.
- **Código:** `MedicaoInformada.fracao`/`pct_quartzo` (`tipos.py`); `pct_leo_poeira` (`leo_resolver.py`, mesma regra dos helpers de R-RX-01) usado na comparação de divergência (`medicoes.py`); `AGENTES_POEIRA_MEDIVEIS` e campos de fração e %quartzo no painel (`web_matriz.py`); origem do RX na revisão com a medição (`_AGENTE_DA_FAIXA` e `_descrever_quantificacao` em `estagios/emissao.py` — a medição do PGR ou do laudo passa a aparecer na origem de todo risco explícito quantificado).
- **Motor sem mudança de regra:** as faixas de R-RX-01 já decidiam por `pct_LT`; a fatia liga o canal.
- **Medido:** 3 pares determinísticos idênticos à `main`, inclusive a origem na revisão.
...
| v195 | 19/09/2026 | Sessão atual (branch `claude/nice-ptolemy-wxk1wo`, mesma sessão da v194; ARQUITETURA/decisão de escopo, sem código de produção): **Nota de aplicação em `D-ARQ-57`** (mesma ID, nenhuma cláusula alterada) — medição aprofundada do que "aceitar cabeçalho CAIXA ALTA" exigiria de fato: banda Função ~50-83pt (bem mais estreita que os 2 witnesses limpos), o único caso N:1 real do documento (pág. 14, 6 cargos) com separador `/` **ausente** entre 2 dos 6 nomes, e cabeçalho que **não repete em toda página** do intervalo (pág. 32 sem `'FUNÇÃO'`/`'TIPO'`) — contrato "recalibra por página" da fatia 5a quebra sem mudança de desenho. Achado que decidiu a sessão: mesmo com a 5b completa para este witness, o documento não ingere hoje (faltam 5c/5d) — já cai em revisão humana, gated by design, sem risco de dado errado. **Decisão do Diovanni: não perseguir a calibração do Hetrin/set-2026 agora** — revisitar quando houver sinal se é template novo do cliente Ricco ou formatação pontual. `DT-(sessão claude/youthful-lamport-3kfkog)-02` segue ABERTA, fatiamento 5a→5b→5c→5d inalterado. Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
| v196 | 19/09/2026 | Sessão atual (branch `claude/nice-ptolemy-wxk1wo`, mesma sessão da v195; IMPLEMENTAÇÃO, pedido do Diovanni): **Nota de aplicação em `D-ARQ-47`** (mesma ID, nenhuma cláusula alterada) — `preparar_composicao` ganha o 1º chamador de produção: `pagina_matriz()` ganha upload avulso de FDS/FISPQ (`accept_multiple_files`), funciona com ou sem PGR, mostra CAS/nome/frases-H + pendências por FDS. Cl.4 preservada (para em `gate_forma`, nenhum bypass de revisão-RT). Fatia 1 de 2 — falta o casamento FDS↔`ProdutoQuimico` do PGR (decisão já tomada: seleção manual na tela, `DT-(sessão claude/nice-ptolemy-wxk1wo)-01` aberta em `PENDENCIAS_CLINICAS.md`). Testado ao vivo (`streamlit run app_matriz_local.py` + upload real de FISPQ extraído do Hetrin/set-2026): extração real localizou a composição, transcrição sem chave virou pendência limpa na tela, sem exceção. 4 testes novos (AppTest), varredura inversa 4/4 confirmada. `mypy --strict` alvo canônico limpo, 49 arquivos, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. Detalhe em HISTORICO (bloco desta sessão). |
| v197 | 19/09/2026 | Sessão atual (branch `claude/nice-ptolemy-wxk1wo`, mesma sessão da v196; ARQUITETURA/bloqueador, sem código): **Nota de consequência observada em `D-ARQ-49`** (mesma ID, nenhuma cláusula alterada) — ao planejar a fatia 2 do plug FDS→produção, medido que `hidratar_pgr` crava `GHEPGR.produtos_quimicos=()` sempre; a extração formal (Parte 2 desta D-ARQ, "FDS apontadas") nunca foi implementada, e a forma concreta real (`GHEVerbatim`/`RiscoVerbatim`, nota 003.BN) não tem apontador-FDS distinto. Sem essa extração, `resolver_composicao`/`executar_com_composicao` (já rodam dentro de `processar_pgr`) nunca têm insumo em produção. Diovanni decidiu não implementar a extração agora (fora do tamanho da fatia 2) nem usar `RiscoVerbatim.agente`/`fonte_geradora` como substituto não-medido — sessão parada, nota espelhada em `DT-(sessão claude/nice-ptolemy-wxk1wo)-01` (`PENDENCIAS_CLINICAS.md`). Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
| v198 | 19/09/2026 | Sessão atual (branch `claude/nice-ptolemy-wxk1wo`, mesma sessão da v197; ARQUITETURA/medição, sem código): **Nota de medição em `D-ARQ-49`** (mesma ID, nenhuma cláusula alterada) — o substituto considerado na v197 (`RiscoVerbatim.agente`/`fonte_geradora` como lista de produto) medido contra o PGR real Consciente/Fascino (`parser_familia_consciente.py`, determinístico, sem LLM): `fonte_geradora="Exposição a tintas e seus componentes."` sozinha agrupa ~15 `agente` distintos (componentes químicos individuais da composição de 1 produto, já decompostos no texto do PGR) — é 1 FDS : N linhas de `agente`, nunca 1:1, inverso do N:1 medido na fatia 5b de `D-ARQ-57` mais cedo nesta sessão. Substituto descartado por dado real, não por cautela — a unidade de casamento certa é grupo por `fonte_geradora`, não produto nomeado (o PGR não nomeia produto). `DT-(sessão claude/nice-ptolemy-wxk1wo)-01` segue ABERTA, só caminho (b) (extração formal, D-ARQ-49 P2) segue nomeado pra retomada. Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
| v199 | 19/09/2026 | Sessão atual (branch `claude/sharp-wozniak-j4596a`, número não atribuído; ARQUITETURA, sem código de produção, pedido explícito do usuário para retomar D-ARQ-49 Parte 2): **Nota de ARQUITETURA PROPOSTA em `D-ARQ-49`** (mesma ID, nenhuma cláusula alterada) — agrupar `produtos_quimicos` por `fonte_geradora` na hidratação (caminho (b) nomeado na v198) é REJEITADO por achado novo: `stage_3_pendencias_estruturais`/R-PGR-04 (`estagios/pendencias_estruturais.py:7-19`) emite pendência BLOQUEANTE `composicao_ausente` para todo `produto.fds is None`, e medição reproduzível contra o Fascino real (`parsear_arquivo`, função já versionada, 19 GHEs/237 riscos) achou grupos FÍSICOS de `n=2` (`Ruido`+`Vibrações`; `Sílica livre`+`Poeira respirável`) contra QUÍMICOS reais de `n=8`/`n=10`/`n≈15` — lacuna limpa NESTE witness, mas nada na estrutura do PGR garante que um documento diferente não combine 3+ agentes físicos sob 1 `fonte_geradora` (as próprias Ruído/Vibração/Radiação aparecem separadas noutros GHEs do mesmo Fascino); cravar corte de `n` sem essa garantia inventa categoria sem fonte (D-ARQ-22), e `agentes.yaml` não tem campo de categoria química/física utilizável como filtro alternativo. Extração automática sem garantia estrutural arriscaria R-PGR-04 falso em GHEs físicos que este witness não mostrou. **Decisão revisada:** `produtos_quimicos` nasce vazio na hidratação (zero mudança em `hidratar_ghe`/`hidratar_pgr`); o RT o povoa explicitamente na tela (fatia 2b), escolhendo o GHE e nomeando o produto ao anexar a FDS — o slot só existe já com `fds` populada, nunca `None` órfão, então R-PGR-04 não dispara por esta via (seu gatilho original — produto apontado sem FDS — segue não-implementado, honestamente). Fatia 2a (split cheap/expensive em `processar_arquivo_pgr`, `orquestracao_pgr.py:232-287`, expõe o `PGR` hidratado antes de `processar_pgr`) sequenciada antes da 2b (UI em `web_matriz.py`). ARQUITETURA PROPOSTA — aguarda ratificação do Diovanni. Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
| v200 | 20/09/2026 | Sessão atual (branch `claude/sharp-wozniak-j4596a`, mesma sessão da v199; ARQUITETURA — ratificação, docs-only): **Ratificação em `D-ARQ-49`** (mesma ID, nenhuma cláusula alterada) — Diovanni ratifica a decisão proposta na v199 (produtos_quimicos criado pelo RT na tela, não extraído automaticamente por `fonte_geradora`; fatiamento 2a→2b, sem reordenar). Gap nomeado pelo Crítico na 2ª rodada (universalidade em indústria química não medida contra witness real) fica aceito, não mitigado — decisão explícita, não omissão. D-ARQ-49 Parte 2 passa de ARQUITETURA proposta para **ARQUITETURA ratificada**, liberada para IMPL a partir da fatia 2a. `DT-(sessão claude/nice-ptolemy-wxk1wo)-01` recebe nota da ratificação. Sem código nesta sessão. Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
| v201 | 20/09/2026 | Sessão atual (branch `claude/blissful-johnson-mdeqn5`, número não atribuído; IMPLEMENTAÇÃO, liberada pela ratificação v200): **Nota de aplicação em `D-ARQ-49`** (mesma ID, nenhuma cláusula alterada) — fatia 2a IMPLEMENTADA: `preparar_pgr_hidratado` extraída em `orquestracao_pgr.py`, parando logo após `hidratar_pgr` e devolvendo `tuple[PGR \| None, tuple[Pendencia, ...]]`; `processar_arquivo_pgr` vira wrapper fino sobre ela + `processar_pgr`, MESMO comportamento externo. Só `orquestracao_pgr.py` tocado, motor intocado, conforme a fronteira especificada. Discriminante: suíte existente, sem teste novo pedido pela ARQUITETURA. Recorte (`test_orquestracao_pgr.py`+`test_web_matriz.py`+`test_documento_matriz.py`): 64 passed, sem alteração de asserção. Suíte completa: **1278 passed, 6 skipped, 0 failed**, delta-zero contra o Baseline v200. `mypy --strict` alvo canônico limpo, 49 arquivos, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. Fatia 2b (UI em `web_matriz.py`) segue aberta. |
| v202 | 20/09/2026 | Sessão atual (branch `claude/003ff-fatia2b`, número não atribuído; IMPLEMENTAÇÃO, liberada pela ratificação v200 — fatia 2b): **Nota de aplicação em `D-ARQ-49`** (mesma ID, nenhuma cláusula alterada) — fatia 2b IMPLEMENTADA em `web_matriz.py`: `_rodar_parse_deterministico` decompõe o que antes era 1 chamada a `processar_arquivo_pgr` em `preparar_pgr_hidratado` (caro, D-ARQ-49 fatia 2a) + `processar_pgr` (barato), expondo o `PGR` hidratado; `CacheMatrizes` ganha o campo aditivo `pgr_hidratado` (molde `chamadas_ia`, fora de `calcular_chave_cache`). Núcleo novo `anexar_produto_e_reprocessar(cache, protocolo, ghe_id, nome_produto, fds)`: cria `ProdutoQuimico(nome, fds)` já com `fds` populada (nunca `None` órfão), anexa ao GHE escolhido via `dataclasses.replace` aninhado (GHE→PGR) e roda `processar_pgr` de novo sobre o PGR mutado — sem tocar `preparar_pgr_hidratado`/PDF/LLM. Casca (`pagina_matriz`): com PGR já carregado (`cache.pgr_hidratado is not None`) e FDS avulsa com composição extraída, oferece `st.selectbox` sobre os GHEs do PGR + `st.text_input` do nome do produto (pré-preenchido com o nome do arquivo da FDS — decisão de tela; `fonte_geradora` não sobrevive à hidratação, `RiscoPGR` não a carrega — trazê-la exigiria reabrir a fronteira ratificada de `preparar_pgr_hidratado`, fora do escopo desta fatia) + botão "Anexar ao GHE selecionado"; sem PGR carregado, comportamento idêntico ao de hoje (fluxo avulso da fatia 1/`D-ARQ-47`). Achado de implementação: `Path(__file__)` não pode ser lido de dentro do CORPO de `pagina_matriz()` — `AppTest.from_function` copia o corpo pra um script temporário e `__file__` resolveria pra esse script, não pra `web_matriz.py`; extraído `_protocolo_padrao()` (módulo-nível, único ponto de `carregar(Path(__file__)...)`) para `_rodar_parse_deterministico` e a casca usarem igualmente. Fronteira respeitada: só `web_matriz.py` + `test_web_matriz.py` tocados; `orquestracao_pgr.py` e o motor (`entrada.py`/`orquestrador.py`/`composicao.py`) intocados — a fatia 2a já bastou, como a ARQUITETURA previu. 6 testes existentes que mockavam `processar_arquivo_pgr` diretamente tiveram o mock decomposto em `preparar_pgr_hidratado`+`processar_pgr` (mesmo comportamento observável, mock adaptado ao refactor da fatia 2a) — nenhuma asserção de saída mudou. 3 testes novos: `test_anexar_produto_e_reprocessar_promove_componente_fase_c` (FDS de tolueno, CAS 108-88-3, anexada a 1 GHE atravessa até `riscos_resolvidos` via `resolver_composicao`+Fase C, reversão nomeada: `anexar_produto_e_reprocessar` devolver o cache original sem rodar `processar_pgr`/sem anexar ao GHE certo); `test_anexar_produto_e_reprocessar_anexa_so_no_ghe_escolhido` (reversão: filtro `ghe.id == ghe_id` trocado por algo que anexa em todo GHE); `test_pagina_matriz_fds_anexada_persiste_entre_reruns_sem_chamada_ia`, casca via `AppTest` — sobe PGR sintético (mock só em `preparar_pgr_hidratado`, `processar_pgr` roda de verdade), anexa FDS de tolueno, confirma `riscos_resolvidos`/`chamadas_ia == 0` e roda um 2º `.run()` sem tocar em nenhum widget para provar persistência (reversão nomeada e exigida pelo prompt: não persistir o PGR mutado em `st.session_state` — o componente sumiria no rerun seguinte). Recorte (`test_orquestracao_pgr.py`+`test_web_matriz.py`+`test_documento_matriz.py`): 67 passed (64 da v201 + 3 novos), sem alteração de asserção pré-existente. Suíte completa (`agente_medico/tests/ tests/`, árvore parada): **1281 passed, 6 skipped, 0 failed**, 738.56s. Delta **+3** exato contra o Baseline v201 (`1278`) — os 3 testes novos listados acima; nenhum teste perdido ou alterado. `mypy --strict` alvo canônico: limpo, **49 arquivos**, delta-zero contra v201 (`montar_fds` adicionado a `__all__` — re-export explícito exigido por `--strict`). Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. D-ARQ-49 Parte 2 (fatias 2a+2b) fecha IMPLEMENTADA — plug FDS→GHE em produção via escolha manual do RT. |
| v203 | 20/09/2026 | Sessão atual (branch `docs/003fg-validacao-ao-vivo-fatia2b`, número não atribuído; CONHECIMENTO — validação ao vivo, sem código de produção): **Nota de validação ao vivo em `D-ARQ-49`** (mesma ID, nenhuma cláusula alterada), pedido do Diovanni pós-merge do PR #350 — a fatia 2b só tinha cobertura via PGR/FDS sintéticos (testes automatizados); esta sessão exercitou a tela real (`pagina_matriz`, via `streamlit.testing.v1.AppTest` — upload literal dos bytes do PDF, interação literal com `selectbox`/`text_input`/`button`, não chamada direta às funções internas) contra um PGR real (`matrizes_originais/PGR - CONSCIENTE... FASCINO (15.07.26).pdf`, família Consciente/Fascino, rota determinística `parsear_arquivo`, D-ARQ-65 — 19 GHEs, **0 chamadas LLM**) e FDS reais do acervo (`fds_originais/`). **Limitação declarada:** o container desta sessão não tem `CHAVE_API_GOOGLE` — o passo de transcrição-LLM da composição da FDS (`preparar_composicao`→`transcrever_fds`) não pôde rodar de verdade; contornado lendo o texto bruto real da seção de composição via `extrair_texto_fds` (determinístico, sem LLM) e montando o `BlocoVerbatim` com o dado **real** ali declarado (CAS/nome/faixa), sem inventar nem aproximar — mock cirúrgico só no ponto que exigia a chave ausente, resto do pipeline (parse do PGR, hidratação, `anexar_produto_e_reprocessar`, `processar_pgr`) roda de produção. **Medido:** das 6 FDS do acervo checadas (`01 - FISPQ_cimento - Ciplan.pdf`, `270 -FISPQ - Adesivo PVC Incolor Tigre.pdf`, `44 - FISPQ_solução limpadora - Amanco.pdf`, `70 - FISPQ_textura textucril - Leinertex.pdf`, `Massa-Corrida.pdf`, `tinta_acrilica.pdf`), **4 CAS têm slug no vocabulário hoje**: `13463-67-7` (Dióxido de Titânio → `dioxido_de_titanio`, tinta acrílica), `78-93-3` (MEK → `metil_etil_cetona`, Adesivo PVC Tigre), `67-64-1` (Acetona → `acetona`, Adesivo PVC Tigre) e `141-78-6` (Acetato de Etila → `acetato_de_etila`, Adesivo PVC Tigre) — os demais CAS reais dessas 4 FDS caem em `vocabulario_ausente` (esperado, é a lacuna já mapeada em `DT-003M-02`, não achado novo). **Correção de método [MEDIDO — mesma sessão, verificado de novo]:** a 1ª medição desta nota citou "só 2 CAS resolvem", checado por `grep -B5` sobre `agentes.yaml` — método falho: blocos de comentário entre o nome do agente e o campo `cas:` (ex. `acetona`/`acetato_de_etila`, 4 linhas de comentário) estouravam a janela de 5 linhas e escondiam o nome do slug. Reconferido com `construir_indice_cas` (função de produção, `resolvedor.py`) sobre os 23 CAS reais das 6 FDS — os 4 acima resolvem, 19 continuam `AUSENTE`. Não muda o restante da nota (achado de convergência de fonte no PGR Fascino, demonstração da Fase C no Almoxarifado): `metil_etil_cetona`/`acetona`/`acetato_de_etila` TAMBÉM já apareciam em `riscos_resolvidos` do GHE-10 antes do anexo, mesma classe de convergência já descrita. Subi `tinta_acrilica.pdf` pela tela, escolhi GHE-16 (PINTURA) no `selectbox` (dropdown mostrou as 19 opções reais do PGR), nomeei o produto e cliquei "Anexar": zero exceção em cada etapa, produto persistiu em `st.session_state` após um 2º `.run()` sem tocar em nenhum widget (a reversão que os testes automatizados já cobrem, aqui confirmada contra dado real). **Achado:** nos GHEs plausíveis (Pintura/dioxido_de_titanio, Encanador/metil_etil_cetona), o componente JÁ aparecia em `riscos_resolvidos` ANTES do anexo — o PGR Fascino itemiza esses mesmos químicos diretamente na própria tabela de risco (achado já documentado em D-ARQ-49 v197/v198), então o anexo não produziu delta observável nesses dois casos — convergência de fonte, não falha. Demonstração isolada (anexar a FDS de TiO2 no GHE-04 ALMOXARIFADO, que não tem esse químico hoje — placement ilustrativo, não realista) confirmou a promoção Fase C de fato: `dioxido_de_titanio` entrou em `riscos_resolvidos` só depois do anexo; nenhum exame novo (regra clínica de `dioxido_de_titanio` não emite exame distinto além do já coberto). Nenhum arquivo de código tocado nesta sessão — puramente diagnóstico, nenhuma regressão possível. De passagem: o usuário subiu 11 FDS reais adicionais nomeadas por cargo (`FDS PINTOR.pdf`, `FDS ENCANADOR.pdf`, `FDS CARPINTEIRO.pdf`, etc., commit `96a15e9`) — mais bem casadas com os GHEs do Fascino que as 6 originais; não exercitadas nesta sessão (fora do pedido, fica para validação futura se necessária). Nenhuma `R-*` criada, alterada ou depreciada; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
| v204 | 20/09/2026 | Sessão atual (branch `docs/003fg-validacao-ao-vivo-fatia2b`, número não atribuído; DADO — sessão de dado, `DT-003M-02(A)`, pedido do Diovanni): **Nota de aplicação em `D-ARQ-36`** (mesma ID, nenhuma cláusula alterada — Parte 2 define os 4 ramos do `gate_cas` que esta sessão só POPULA) — 19 slugs novos em `agente_medico/protocolo/vocabulario/agentes.yaml`, cobrindo os CAS reais das 6 FDS medidas no acervo (`fds_originais/`, validação ao vivo v203) que caíam em `vocabulario_ausente`: 8 do Cimento Ciplan, 3 da Tinta Acrílica, 2 do Adesivo PVC Tigre, 6 conservantes/biocidas da Textura Leinertex/Massa Corrida (lista completa em `DT-003M-02`, `PENDENCIAS_CLINICAS.md`). Fonte de `is_carcinogeno_iarc`: IARC Monographs "List of Classifications" — domínio oficial bloqueado pelo proxy de egresso desta sessão, verificado via `WebSearch` cruzando SDS de fabricante/PubChem/InChem/ChemicalBook; **nenhum dos 19 é carcinógeno IARC**. `tem_lt=true` só em `hidroxido_de_amonia` (Quadro 1, Anexo 11 NR-15); demais `false`. **Achado colateral — 2 CAS malformados na FDS real** (aluminato tricálcico `1242-78-3`, N-octil isotiazolinona `26530-20-2`): FALHAM `cas_bem_formado`, defeito de OCR/transcrição do PDF original — usado o CAS correto (dígito confere, fonte externa) pro slug; uma FDS real repetindo o CAS malformado continua caindo em `cas_invalido` (D-ARQ-36 ramo c), comportamento correto do gate, registrado em comentário. Cobertura CAS (`scripts/medir_painel`): **50/80 (62%) → 69/99 (70%)**. **Consequência medida sobre fixtures existentes:** a fixture real `fds_t65.py` (3 FDS reais já versionadas, Cimento Ciplan/Tinta Acrílica/Adesivo PVC Tigre) teve TODO CAS válido remanescente coberto pelos 19 slugs novos — ramo (b) zerou nela (0 candidatos, antes >=1). 6 testes em `test_composicao_propaga_pendencias.py`/`test_integracao_composicao_fase_c.py` tinham asserção ancorada nesse estado antigo (ex.: "Copolímero de PVC fica `agente=None`", "cimento não promove nenhum componente") — reescritos com reversão nomeada, refletindo o novo estado real (ex.: adesivo promove 5 riscos químicos, não 3; cimento promove 6 de 8). Mecanismo do ramo (b) em si (não a cobertura desta fixture) segue coberto por `test_resolvedor.py::test_gate_ramo_b_*` (índice sintético, independente do vocabulário real). Guards de inventário do resolvedor de termos também atualizados (`test_indice_real_tem_125_entradas`→`_144_entradas`, +19 formas; gabarito do vigia fuzzy ganhou o par `silicato_dicalcico`/`silicato_tricalcico`, revisado e aceito, nenhum com `fuzzy_permitido`). Recorte tocado (`test_vocabulario.py`+`test_protocolo_carregamento.py`+`test_resolvedor.py`+`test_resolvedor_termos.py`+`test_composicao_propaga_pendencias.py`+`test_integracao_composicao_fase_c.py`+11 arquivos de FDS/transcrição que consomem as mesmas fixtures reais): **139 passed** (123+16), sem alteração de asserção fora do nomeado acima. Suíte completa (`agente_medico/tests/ tests/`, árvore parada, rodada 2× — a 1ª contra os testes ainda não corrigidos achou os 6 vermelhos acima e foi encerrada antes de terminar; a 2ª mede o estado final): **1281 passed, 6 skipped, 0 failed**, 716.04s — **delta-zero exato** contra o Baseline v202 (1281): as reescritas trocaram asserção, não contagem de teste (6 renomeados 1:1, nenhum novo/perdido). `mypy --strict` alvo canônico: limpo, **49 arquivos**, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada — (A) é dado puro, `materialidade()`/Fase C já liam a flag do `Componente` (D-ARQ-34 P4) antes desta sessão. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. `DT-003M-02` (A) PARCIALMENTE RESOLVIDA — recorte medido coberto, universo maior de FDS reais (11 FDS por-cargo do commit `96a15e9`, ainda não examinadas) segue aberto. |
| v205 | 20/09/2026 | Sessão atual (branch `docs/003fh-fds-por-cargo`, número não atribuído; DADO — sessão de dado, `DT-003M-02(A)` 2ª leva, pedido do Diovanni): **Nota de aplicação em `D-ARQ-36`** (mesma ID, nenhuma cláusula alterada) — examinadas as 11 FDS reais por-cargo do acervo (`fds_originais/`, commit `96a15e9`); dedup por hash (`sha256sum`) reduz a 8 documentos distintos: Adesivo PVC Tigre (Almoxarife/Encanador/Montador — mesma composição da 1ª leva v204, zero CAS novo), Água Sanitária Zulu (Aux. Serviços Gerais/Serviços Gerais), Cimentcola Interno Quartzolit (Azulejista — cimento Portland já coberto), Desmoldante Concentrado (Carpinteiro — sem composição declarada na FDS), Impermeabilizante Asfáltico (Impermeabilizante avulso + páginas 13+ de `FDS PINTOR.pdf`), Eletrodo 60.13 (Montador de Estruturas Metálicas/Soldador). **9 slugs novos**: `hipoclorito_de_sodio` (7681-52-9), `carbonato_de_sodio` (497-19-8), `asfalto` (8052-42-4), `ferro` (7439-89-6), `feldspato` (68476-25-5), `silicato_de_potassio` (1312-76-1), `bentonita` (1302-78-9), `celulose` (9004-34-6), `carbonato_de_potassio` (584-08-7) — todos com CAS bem-formado na FDS real (`cas_bem_formado`), sem malformação desta vez. **Único carcinógeno IARC da leva:** `asfalto` — Monografia IARC Vol.103 (2013), "Bitumens and Bitumen Emissions": exposição ocupacional a betume oxidado+emissões (telhado) = Grupo 2A; betume não-oxidado/duro+emissões (pavimentação/massa asfáltica) = Grupo 2B; a FDS declara "Asfalto" genérico sem especificar a classe — ambas mapeiam a `true` por convenção deste vocabulário (Grupo 1/2A/2B). Manganês (7439-96-5) e sílica/quartzo (14808-60-7), também presentes no Eletrodo 60.13, JÁ tinham slug (`manganes`/`silica`) — confirmado antes de pesquisar, sem trabalho duplicado. **Limitação medida:** as páginas 0-11 de `FDS PINTOR.pdf` (produto "BLASCOR", provável tinta) são PDF só-imagem — `pdfplumber` não extrai texto, sem composição legível nesta sessão (exigiria OCR, fora de escopo desta fatia de dado; registrado, não escondido). Cobertura CAS (`scripts/medir_painel`): **69/99 (70%) → 78/108 (72%)**. **Consequência sobre fixtures/testes:** nenhuma — os 9 slugs novos não aparecem na fixture `fds_t65.py` (Cimento Ciplan/Tinta Acrílica/Adesivo PVC Tigre, já 100% coberta desde v204); recorte tocado (`test_vocabulario.py`+`test_protocolo_carregamento.py`+`test_resolvedor.py`+`test_resolvedor_termos.py`+`test_composicao_propaga_pendencias.py`+`test_integracao_composicao_fase_c.py`): **139 passed**, sem alteração de asserção fora do guard de inventário (`test_indice_real_tem_144_entradas`→`_153_entradas`, +9 formas, nenhum par fuzzy novo no vigia). Recorte estendido (11 arquivos de FDS/transcrição/web_matriz/orquestracao_pgr que consomem as mesmas fixtures reais): **162 passed, 3 skipped**. Suíte completa (`agente_medico/tests/ tests/`, árvore parada): **1281 passed, 6 skipped, 0 failed**, 713.61s — delta-zero exato contra o Baseline v204 (1281; rename de teste, não contagem). `mypy --strict` alvo canônico: limpo, **49 arquivos**, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada — (A) é dado puro. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. `DT-003M-02` (A) segue PARCIALMENTE RESOLVIDA — 17 FDS medidas ao todo entre as duas levas; universo maior fica aberto, sem candidato conhecido pendente além do OCR do `FDS PINTOR.pdf`. |
| v206 | 21/09/2026 | Sessão atual (branch `claude/nice-fermat-xahkji`, número não atribuído; IMPLEMENTAÇÃO, sem sessão de ARQUITETURA prévia — proposta já fechada na própria DT): **Nota de aplicação em `D-ARQ-34` Parte 1 e `D-ARQ-43` Parte 2** (mesma ID nas duas, nenhuma cláusula alterada) — fecha `DT-(sessão branch docs/003fi-achado-gate-forma-faixa)-01`: `_SEPARADOR_FAIXA_FALLBACK = re.compile(r"\s+a\s+|\s+", re.IGNORECASE)` em `motor/transcricao_fds.py`, consultado por `parsear_faixa` só quando o separador primário (hífen/en-dash, D-ARQ-43 P2) não encontra 2 partes — nunca compete com ele, então as âncoras já ratificadas (`"0,2 – 0,05"` etc.) continuam pelo caminho antigo. Motivo: `pdfplumber` extrai as duas colunas da tabela de composição como números soltos em FDS reais (achado medido na sessão anterior, 3 fabricantes distintos), e `gate_forma` (`transcritor_fds.py`, reusa `parsear_faixa` por construção) reprovava o bloco inteiro, bloqueante, mesmo com CAS/nome íntegros. LLM/prompt intocados. 6 testes novos (3 casos reais como fixture direta — `"15 19"`, `"30 70"`, `"35 a 50"` — + 2 de fronteira + 1 no nível do gate); varredura inversa (revogar só o fallback) derruba exatamente os 4 testes que dependem dele, preserva os 2 que não dependem, nenhum teste pré-existente se move. Recorte tocado (`test_transcricao_fds.py`+`test_transcritor_fds.py`+`test_montagem_verbatim.py`+`test_revisao_verbatim.py`+`test_orquestracao_fds.py`+`test_transcritor_gemini.py`+`test_cli_fds.py`+`test_web_fds.py`+`test_web_matriz.py`+`test_composicao_propaga_pendencias.py`+`test_integracao_composicao_fase_c.py`): **156 passed, 3 skipped** (skips são `@requer_pdfs`, ambiente sem os PDFs de `fds_originais/`, DH-003ET-01). Suíte completa (`agente_medico/tests/ tests/`, árvore parada): **1287 passed, 6 skipped, 0 failed**, 764.80s. Delta **+6** exato contra o Baseline v205 (`1281`) — os 6 testes novos, nenhum removido/renomeado. `mypy --strict` alvo canônico: limpo, **49 arquivos**, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada; nenhum `.yaml` de vocabulário tocado; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. `DT-(sessão branch docs/003fi-achado-gate-forma-faixa)-01` RESOLVIDA (`PENDENCIAS_CLINICAS.md`). |
| v207 | 21/09/2026 | Sessão atual (branch `claude/nice-fermat-xahkji`, número não atribuído; IMPLEMENTAÇÃO, achado pós-merge do PR #354): **Nota de aplicação em `D-ARQ-34` Parte 1** (mesma ID, nenhuma cláusula alterada) — fecha `DT-(sessão claude/nice-fermat-xahkji, achado pós-PR #354)-01`: Diovanni testou o app publicado de novo com FDS reais de outro PGR ("CMO Residencial Verdes Mares", PDFs subidos ao acervo via commits `8695842`/`111ee0e`) e achou `DESMOLD SIKA - (GHE 05 CARPINTARIA).pdf` reprovando no gate — `extrair_texto_fds` sobre o PDF real confirma `"dazomete (ISO) 533-74-4 >= 0.1 - < 1"`, uma **faixa dupla-desigualdade** (`>=`/`<` nos dois lados), classe distinta do separador ausente da v206: há hífen, mas `bruto.startswith(">")` (D-ARQ-34 P1, semi-aberta simples) captura o `">="` inicial antes de qualquer split e perde o teto. `_FAIXA_COMPOSTA = re.compile(r"^>=?\s*([\d.,]+)\s*[-–]\s*<=?\s*([\d.,]+)$")` em `motor/transcricao_fds.py`, checada em `parsear_faixa` ANTES dos ramos de semi-aberta simples — `=?` aceita operador estrito ou "ou-igual", mesmo vocabulário que D-ARQ-34 P1 já usa isoladamente, não introduz conceito novo. Escopo deliberadamente estreito: só `>`/`>=` e `<`/`<=` ASCII (nenhuma FDS medida usa `≥`/`≤` unicode — não adicionado por antecipação). LLM/prompt intocados; `gate_forma` não muda. 5 testes novos (caso real do dazomete, operador estrito, teto ou-igual, confirmação de que as semi-abertas simples `"> 1"`/`"< 5"` continuam intocadas, 1 no nível do gate com o bloco real); varredura inversa (revogar só o branch `_FAIXA_COMPOSTA`) derruba exatamente 4/5, preserva o que não depende dele, nenhum teste pré-existente se move. Recorte tocado (mesmos 11 arquivos da v206): **161 passed, 3 skipped**. Suíte completa (`agente_medico/tests/ tests/`, árvore parada): **1292 passed, 6 skipped, 0 failed**, 746.98s. Delta **+5** exato contra o Baseline v206 (`1287`) — os 5 testes novos, nenhum removido/renomeado. `mypy --strict` alvo canônico: limpo, **49 arquivos**, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada; nenhum `.yaml` de vocabulário tocado; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. `DT-(sessão claude/nice-fermat-xahkji, achado pós-PR #354)-01` RESOLVIDA (`PENDENCIAS_CLINICAS.md`). |
| v208 | 21/09/2026 | Sessão atual (branch `claude/nice-fermat-xahkji`, número não atribuído; IMPLEMENTAÇÃO, achado pós-merge do PR #355): **Nota de aplicação em `D-ARQ-34` Parte 1** (mesma ID, nenhuma cláusula alterada) — generaliza a mesma DT da v207 (`PENDENCIAS_CLINICAS.md`, 2ª leva): Diovanni subiu FDS reais de outro PGR ("CMO Residencial Aurora", commit `683f5f7`) e `Fundo Zarcão- PINTURA ESMALTE SINTÉTICO- PINTOR.pdf` reprovava o bloco de "Destilados de Petróleo" — `extrair_texto_fds` confirma `"10 - <50"`, faixa composta **assimétrica** (só o teto tem operador `"<"`, o piso é número puro), classe que o `_FAIXA_COMPOSTA` da v207 não cobria (exigia operador nos DOIS lados). Regex generalizado de `^>=?...<=?...$` para `^(?:>=?)?\s*([\d.,]+)\s*[-–]\s*(?:<=?)?\s*([\d.,]+)$` — cada lado com operador opcional independente — gated por `">" in bruto or "<" in bruto` antes de tentar o match, preservando intocado o caminho antigo para o caso sem nenhum operador (mesmo princípio de não competir com o separador primário, já usado no fallback de espaço/`" a "` da v206). 3 testes novos (`"10 - <50"` → `(10.0, 50.0)`, achado real; `">10 - 50"` → `(10.0, 50.0)`, simétrico ao achado mas com o piso, mesma classe de forma; 1 no nível do gate com o bloco real de "Destilados de Petróleo"); varredura inversa (reverter só o regex para a forma simétrica antiga) derruba exatamente os 3 novos, preserva os 5 da v207 e todo o resto. Recorte tocado (mesmos 11 arquivos da v206/v207): **164 passed, 3 skipped**. Suíte completa (`agente_medico/tests/ tests/`, árvore parada): **1295 passed, 6 skipped, 0 failed**, 747.13s. Delta **+3** exato contra o Baseline v207 (`1292`) — os 3 testes novos, nenhum removido/renomeado. `mypy --strict` alvo canônico: limpo, **49 arquivos**, delta-zero. Nenhuma `R-*` criada, alterada ou depreciada; nenhum `.yaml` de vocabulário tocado; nenhuma D-ARQ nova. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. **Deslize de processo, auto-reportado:** o índice D-ARQ não foi regenerado no primeiro passe desta linha (v208 escrita em disco antes de rodar `scripts/gerar_indice_darq`) — exatamente a cláusula fixa do CLAUDE.md (precedente `c89f569`/003.EF, `973a343`/003.EG). A 1ª corrida da suíte completa pegou o defeito (`test_gerar_indice_darq.py::test_indice_em_disco_nao_divergiu` e `test_medir_painel.py::test_medir_indice_darq_sincronizado_com_o_disco`, **2 failed**, 1293 passed, mesma corrida de 747.58s) — índice regenerado, suíte refeita do zero, o número acima (1295 passed, 0 failed) é da 2ª corrida, limpa. `DT-(sessão claude/nice-fermat-xahkji, achado pós-PR #354)-01` segue RESOLVIDA (nota adicional na mesma entrada, `PENDENCIAS_CLINICAS.md`). |
| v209 | 22/09/2026 | Sessão atual (branch `claude/vigilant-faraday-d8a4wn`, número não atribuído; DADO — reabertura parcial de `DT-003BA-01`, autorizada pelo Diovanni): **Nota de aplicação em `D-ARQ-12`** (mesma ID, nenhuma cláusula alterada) — campo novo `enquadramento_3048` em 27 slugs de `agentes.yaml`, dado puro sem consumidor no motor, fonte Decreto 3.048/1999 Anexo IV (texto oficial compilado, `planalto.gov.br`, fornecido pelo Diovanni após bloqueio de egresso do domínio nesta sessão, conferido item a item). Achado: o `dec_3048` do motor legado (`modules/modulo_engenharia.py`) errava em tolueno/xileno (rotulados 1.0.19, que na verdade só cita "diisocianato de tolueno (TDI)") e em `dissulfeto_de_carbono` (rotulado "Não Enquadrado", consta literalmente em 1.0.11) — corrigidos, não replicados. Escopo eSocial (Tab 24) e demais colunas de engenharia de `DT-003BA-01` **seguem FORA**, não tocados. 3 testes novos com reversão nomeada (`test_vocabulario.py`): valores fixos por amostra, regressão específica tolueno/xileno=null, guarda de forma sobre as 27 entradas. Nenhuma `R-*` criada, alterada ou depreciada; nenhum campo novo em `Risco`; nenhuma emissão. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
| v210 | 23/09/2026 | Sessão atual (branch `claude/hopeful-newton-yjv3k7`, número não atribuído; IMPLEMENTAÇÃO — fecha `DT-(sessão claude/hopeful-newton-yjv3k7)-01`): **Andamento em `D-ARQ-57`** (mesma ID, nenhuma cláusula alterada; toca também `D-ARQ-65` cl.5 pela separação de cargos) — forma 6 do repertório GHE (separador antes do número: Porto Araras I, Vila Brasil Escritório) e separação de cargos por código CBO em vez da corrida final de `[\d\x00\s-]+`. 9 testes novos (3 parametrizados em `test_extracao_pgr.py`, 6 em `test_parser_familia_consciente.py`), varredura inversa 9/9. Suíte completa (`agente_medico/tests/ tests/`, árvore parada): **1307 passed, 6 skipped, 0 failed**, 853.93s. `mypy --strict` alvo canônico: limpo, **49 arquivos**. PROTOCOLO inalterado, segue **v94**; decisões inalteradas em **85**. Índice D-ARQ regenerado. |
