...
#### R-BIO-05 — Risco baixo ou irrelevante no PGR dispensa o indicador IBE/EE: menção documental no lugar do exame `[INTERPRETADO — prioridade na revisão de saída]`

Agente do **Quadro 1 (IBE/EE)** cujo risco o PGR classifica como **BAIXO** ou **IRRELEVANTE** na avaliação qualitativa P×S da própria linha → o indicador biológico de R-BIO-04 **não é emitido**; a matriz leva, na linha do cargo, a observação *"risco baixo no PGR para <agente> — incluir menção no PCMSO; não solicitado: <exame>"*, uma por agente.

- **Condição de dispensa:** todo risco do agente no GHE traz nível listado. Basta uma linha do mesmo agente em MODERADO ou acima, ou sem nível (PGR sem avaliação P×S, rota LLM/card, risco implícito, componente de FDS), para o indicador sair normalmente. Silêncio do PGR nunca vira dispensa.
- **Escopo:** só Quadro 1 (IBE/EE, 42 regras `R-BIO-04-*`). O Quadro 2 (IBE/SC — chumbo, cádmio, fluoretos, inseticidas inibidores da colinesterase) segue emitindo em qualquer nível: o indicador ali tem significado clínico (7.5.19.5), não só de exposição, e nenhum caso medido o dispensa. Fora também `R-BIO-03` (manganês, via NR-15) e `R-PKG-BZ` (benzeno, carcinógeno).
- **IRRELEVANTE junto de BAIXO** `[INTERPRETADO]`: a decisão do Diovanni fala em "risco baixo"; IRRELEVANTE fica abaixo de BAIXO na mesma matriz P×S, e dispensar em BAIXO mas emitir em IRRELEVANTE seria incoerente. Caso medido: Porto Araras I, pintor, 2-butoxietanol anotado "irrelevante" pela Dra. Patrícia, gabarito sem BAA.

**Origem.** `DT-003EB-02`. Decisão do Diovanni (23/09/2026): "segue o da Dra. Carolini". Anotação verbatim no gabarito Fascino SPE 0030 (Dra. Carolini, 08/07/26), coluna FUNÇÃO: *"Encanador (Incluir no word do PCMSO, risco baixo no PGR para acetona e metiletilcetona)"*; *"Pintor (Incluir no word do PCMSO, risco baixo no PGR para destilados-petróleo, tolueno, metiletilcetona e xileno)"*. **Precedente em sentido contrário, declarado (D-ARQ-81 cl.1):** Porto Araras I (Dra. Patrícia, 06/07/26) pede `Metil-etil-cetona (PER 6 meses)` para o pintor com o risco classificado como baixo — as duas médicas divergem; a regra segue a da Dra. Carolini por decisão, não por convergência. Viés: construção civil, 2 médicas, 3 PGRs pareados.

**Relação com a norma — por que não é `[DERIVADO]`.** A NR-07 (Portaria MTP 567/2022) põe o indicador IBE/EE como obrigatório no periódico do exposto (7.5.15, base de R-BIO-04) e amarra o PCMSO aos riscos **identificados e classificados** no PGR. Se o texto vigente condiciona o monitoramento à classificação do risco — o que tornaria esta regra `[DERIVADO]` — ou se o exige para todo exposto — o que a tornaria **menos protetiva que a norma**, e não apenas mais estreita que o precedente — **não foi conferido nesta sessão**: `www.gov.br` negado pela política de rede do container `[A CONFERIR — D-ARQ-69]`. É o item a inspecionar primeiro na revisão de saída desta regra.

**Implementação.** Chave `mencao_documental: {regra: R-BIO-05, niveis_risco: [IRRELEVANTE, BAIXO]}` nas 42 `R-BIO-04-*` do Quadro 1 (`regras.yaml`); desvio em `stage_5_emissao` (`_nivel_dispensa`, `motor/estagios/emissao.py`); `Observacao` em `MatrizGHE.observacoes` (`motor/tipos.py`); célula de observação em `documento_matriz.py` e linha na apresentação de revisão (`apresentacao_matriz.py`). O carregador recusa a chave em regra cujo `quando` não é slug de agente ou com nível que o parser não produz. Nível vem de `RiscoPGR.nivel_risco` — hoje só a rota determinística (família Consciente) o preenche (DT-003EC-01); nas demais rotas o indicador segue saindo. R-BIO-04 mantém ID e status: a família continua sendo a regra, esta é a exceção por nível.
...
| v97 | 24/09/2026 | Branch `claude/eager-fermat-txbn7h` (IMPLEMENTAÇÃO, decisão do Diovanni de 23/09/2026): **`R-BIO-05` CRIADA** (§5.9) — agente do Quadro 1 (IBE/EE) com todo risco classificado IRRELEVANTE/BAIXO na matriz P×S do PGR → indicador de R-BIO-04 não emitido; observação de menção documental na linha do cargo, uma por agente. `[INTERPRETADO]`, conferência da NR-07 vigente `[A CONFERIR — D-ARQ-69]` (gov.br negado pela rede). Fecha `DT-003EB-02`. Quadro 2 (IBE/SC), R-BIO-03 e R-PKG-BZ fora. Efeito medido (`comparar_matriz_gabarito`, `main` × working tree): superemissão 7→0 (Fascino, Dra. Carolini), 1→0 (Porto Araras I); **subemissão 0→6 (Porto Araras I) e 9→13 (Vila Brasil) — a Dra. Patrícia pede o indicador em risco BAIXO em 10/10 células medidas**. `[BLOQUEADOR — reportado ao Diovanni, decisão pendente: manter, restringir a IRRELEVANTE, ou parametrizar por médico]` |
