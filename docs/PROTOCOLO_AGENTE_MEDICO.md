...
**Faceta B segue ABERTA → 003.DX.** fuzzy `Silício`→`silica` (dist 2, FUZZY-FP) é mecanismo, não dado — não tocado nesta sessão. Vigia `test_vigia_pares_fuzzy_chaves_longas` (003.DN) manteve verde sob +6 aliases: nenhuma colisão fuzzy nova entre slugs distintos.
