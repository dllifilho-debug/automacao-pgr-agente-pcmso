...
### DT-(sessão claude/nice-ptolemy-wxk1wo)-01 — Casamento manual FDS↔produto do PGR: falta o elo que liga a composição extraída ao `ProdutoQuimico` certo `[ABERTA — fatia 2 do plug FDS→produção]`
...
1. Expor a lista de `produtos_quimicos` (nomes) do PGR já processado pra tela escolher — hoje `_rodar_parse_deterministico`/`processar_arquivo_pgr` devolvem `Resultado` (matrizes já resolvidas), não o `PGR` hidratado com a lista de produtos antes da resolução de composição.
...
3. UI: `st.selectbox` por FDS enviada, listando os produtos do PGR já parseado; sem PGR processado, a FDS fica só no modo avulso (fatia 1, sem casamento — comportamento já existente).

**Status:** ABERTA. Não medido/desenhado — a lista acima é levantamento de superfície, não arquitetura fechada. Não-bloqueante: a fatia 1 (extração avulsa) já entrega valor sozinha. `[DERIVADO — pedido do Diovanni nesta sessão; leitura do código de `orquestracao_pgr.py`/`web_matriz.py`/`motor/entrada.py` desta sessão, sem medição contra PGR+FDS reais juntos]`
