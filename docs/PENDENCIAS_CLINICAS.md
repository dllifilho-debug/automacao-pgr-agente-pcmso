...
### DT-(sessão claude/nice-ptolemy-wxk1wo)-01 — Casamento manual FDS↔produto do PGR: falta o elo que liga a composição extraída ao `ProdutoQuimico` certo `[ABERTA — ARQUITETURA ratificada, liberada para IMPL da fatia 2a]`
...
**Status:** ABERTA — fatia 2a IMPLEMENTADA, resta a fatia 2b (UI em `web_matriz.py`: RT escolhe o GHE e nomeia o produto ao anexar a FDS avulsa, usando `preparar_pgr_hidratado` pra cachear o `PGR` entre reruns do Streamlit sem reprocessar PDF/LLM). `[MEDIDO — parsear_arquivo real contra o Fascino + leitura de estagios/pendencias_estruturais.py/PROTOCOLO_AGENTE_MEDICO.md §R-PGR-04, sessão claude/sharp-wozniak-j4596a; ratificação do Diovanni na mesma sessão; fatia 2a medida na sessão claude/blissful-johnson-mdeqn5]`
