...
**Status:** ABERTA. Não-bloqueante: a fatia 1 (extração avulsa, `PR #345`) já entrega valor sozinha. `[DERIVADO — pedido do Diovanni nesta sessão; leitura de `orquestracao_pgr.py`/`web_matriz.py`/`motor/entrada.py`/`motor/hidratacao.py`/`D-ARQ-49`; sem medição contra PGR+FDS reais juntos; decisão de parar é do Diovanni]`
