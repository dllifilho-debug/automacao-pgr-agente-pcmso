...
**Tiragem corrente:** 003.BN · 06/07/2026
**Baseline:** main `94bd5c4` · 577 passed, 3 skipped · mypy `--strict` delta-zero (46 pré-existentes) · PROTOCOLO v41 · DECISOES v88
...
**Headline:** a produção **não está travada por falta de regra clínica nem mais pelo transcritor-FDS** (lado-químico fim-a-fim construído e validado ao vivo, 003.BH/BI) — está travada por **parse de PGR** (esqueleto GHE/cargo/risco: transcrição-LLM tem esqueleto no main — 003.BN, tipo + invocação + gate, LLM mockado; restam cliente real e termo→slug, D-ARQ-49/50) e por **índice CAS/vocabulário químico raso** (a maioria dos componentes de FDS real ainda cai em ramo 0/AUSENTE por falta de slug, DT-003M-02). Nenhuma regra clínica nova move essa restrição.
...
| Parse de PGR (esqueleto GHE/cargo/risco físico) | Em construção | parse-doc + recorte-GHE + tipo transcrito/invocação/gate no main (`extrair_texto_pgr`/`recortar_blocos_ghe`/`transcrever_ghes`, 003.BL/BM/BN, LLM mockado); restam cliente-LLM real e termo→slug (D-ARQ-49/50) — maior massa |
...
| Parse de PGR bruto → `tipos.PGR` (esqueleto GHE/cargo/risco físico) | **parse-doc + recorte-GHE + tipo transcrito/invocação/gate construídos** (`extrair_texto_pgr`/`recortar_blocos_ghe`/`transcrever_ghes`/`gate_forma_ghe`, 003.BL/BM/BN, LLM sempre mockado); cliente-LLM real e termo→slug **0 código** (D-ARQ-49/50) — maior massa restante |
...
5. **A declarar no kickoff (proposta do Arquiteto, 003.BN):** 4ª fatia do parse-PGR — cliente-LLM real do transcritor-GHE (molde `TranscritorGemini`, validação ao vivo contra o Viverde) **ou** resolvedor termo→slug (D-ARQ-50 P2).
6. **Fatia A:** plugar `executar_com_composicao`/`montar_fds_revisado` no pipeline real (CLI/Streamlit), sobre índice CAS populado.
7. **`name→slug`:** decisão de arquitetura a tomar (D-ARQ-36 Parte 1) — destrava agentes sem CAS atômico.
8. **META higiene DH-003M-01:** 4ª recorrência de markdown cru, ainda pendente.
