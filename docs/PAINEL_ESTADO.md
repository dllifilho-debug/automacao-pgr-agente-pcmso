...
**Tiragem corrente:** 003.EN · 01/08/2026
**Baseline:** main `a04986e` (PR #277, D-ARQ-72) + R-PSY-02 materializada nesta branch (`feat/003en-psicossocial`) · 1027 passed, 6 skipped · PROTOCOLO v83 · DECISOES v161 (inalterado, esta sessão não cria D-ARQ) — hash do merge desta branch fica pendente até o Diovanni mergear
...
**Instrumento oficial nesta tiragem (disco, branch `feat/003en-psicossocial` sobre `main a04986e`):** regras **22/42** ativas pelo instrumento (52%) / **21** pela intenção do painel (ver acima e DH-003EC-01(b)) — **movimento desde 003.EI: 21→22 / 20→21** (R-PSY-02 materializada por IMPLEMENTAÇÃO, R-PSY-01 corretamente excluído por ser DEPRECATED, checado por script: `medir_cobertura_clinica()` = `(22, 42)`) · vocabulário/CAS **50/79** slugs (63%), **inalterado desde 003.DK** (esta sessão populou 2 slugs novos de exame em `exames.yaml`, sem campo `cas` — não conta para este índice) · índice D-ARQ **sincronizado, inalterado** (esta sessão não cria D-ARQ, não dispara regeneração) · suíte **1025→1027 passed, 6 skipped** (+2 testes novos, D-ARQ-66 cl.4/vocabulário), medida com árvore parada; `mypy --strict agente_medico/motor agente_medico/tests/invariantes.py` delta-zero, 34 arquivos.
