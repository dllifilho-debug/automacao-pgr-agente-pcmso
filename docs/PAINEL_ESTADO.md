...
**Tiragem corrente:** 003.EO + EMENDA 1 · 01/08/2026
**Baseline:** main `2b7efe4` (PR #278 — merge de 003.EN/R-PSY-02; corrige o resíduo da tiragem anterior, que registrava o hash desse merge como pendente) + emissor de saída materializado nesta branch (`feat/003eo-emissor`) · 1039 passed, 6 skipped · PROTOCOLO v83→v84 · DECISOES v161→v162 (D-ARQ-73 CRIADA)
...
|---|---|---|
| Quanto da regra clínica está no código? | **22 de 42** ativas pelo instrumento (52%) — **21 pela intenção do painel**, medido por `scripts/medir_painel.py` | Movimento 21→22 desde 003.EI: R-PSY-02 entra por IMPLEMENTAÇÃO (`regras.yaml` `id: R-PSY-02`, incondicional via `todo_trabalhador`, D-ARQ-66) — footprint executável real. R-PSY-01 sai do denominador por virar `[DEPRECATED]` (sucedida por R-PSY-02); denominador segue **42** porque a entrada de R-PSY-02 compensa a saída. Verificado de disco (003.EN): `R-PSY-01 in ativos` = False — o filtro de header do instrumento (`_ids_ativos_protocolo`) já exclui headers "DEPRECATED" antes da interseção, então a citação "Sucede R-PSY-01 (DEPRECATED)" na `base_normativa` de R-PSY-02 não infla o número (checado por script; nova instância de DH-003EC-01(b), mesma classe de R-ESP-01/R-ESP-02 em 003.EI). R-TEMP-01 segue o único caso vivo de citação-sem-execução — a intenção do painel (regra executável) é **21**. Dois números, registrados, não escolhidos em silêncio |
...
**Instrumento oficial nesta tiragem (disco, branch `feat/003eo-emissor` sobre `main 2b7efe4`):** regras **22/42** ativas pelo instrumento (52%) / **21** pela intenção do painel — **inalterado**, 003.EO não criou, alterou nem depreciou nenhuma R-* (cláusula fixa do prompt) · vocabulário/CAS **50/79** slugs (63%), **inalterado** (esta sessão só populou `ordem_exibicao` — dado de apresentação, não `cas` — em `exames.yaml`) · índice D-ARQ **regenerado** (`python -m scripts.gerar_indice_darq`, DECISOES v161→v162, **D-ARQ-73 nova** — emissor de saída no formato do escritório) · suíte **1027→1039 passed, 6 skipped** (+12 testes novos: 2 fatia 1/MatrizGHE, 1 unicidade de `ordem_exibicao`, 9 `documento_matriz.py` — expansão/HTML/DOCX), medida com árvore parada; `mypy --strict agente_medico/motor agente_medico/superficie agente_medico/tests/invariantes.py` limpo, **42 arquivos** — não comparável ao baseline 34 (`motor/`+`invariantes.py` só): o comando desta sessão passou a incluir `superficie/` pela 1ª vez (7 preexistentes + `documento_matriz.py`); 42 é o novo baseline, não regressão.
...
- **Denominador:** 42 IDs ativos no PROTOCOLO (header = status; R-PSY-01 sai por virar `[DEPRECATED]`, R-PSY-02 entra — 42 inalterado). Medido por `scripts/medir_painel.py` (003.DE), instrumento oficial desde então.
