...
|---|---|---|
| Quanto da regra clínica está no código? | **21 de 42** ativas pelo instrumento (50%) — **20 pela intenção do painel**, medido por `scripts/medir_painel.py` | Movimento 20→21 desde 003.ED: R-ESP-02 entra por IMPLEMENTAÇÃO (`regras.yaml` `id: R-ESP-02` + `predicados.py::_pnos`, D-ARQ-69) — footprint executável real, não citação. Verificado de disco nesta tiragem (003.EI): R-ESP-01, DEPRECATED, é corretamente EXCLUÍDO do numerador — o filtro de header do instrumento (`_ids_ativos_protocolo`) descarta qualquer header com "DEPRECATED" antes da interseção, então a citação "Sucede R-ESP-01 (DEPRECATED)" na `base_normativa` de R-ESP-02 não infla o número (checado por script, `R-ESP-01 in ativos` = False). R-TEMP-01 segue o único caso vivo de citação-sem-execução (DH-003EC-01(b), inalterado) — a intenção do painel (regra executável) é **20**. Dois números, registrados, não escolhidos em silêncio |
...
**Instrumento oficial nesta tiragem (disco, branch `feat/003ei-fechamento-docs` sobre `main 6edee2c`):** regras **21/42** ativas pelo instrumento (50%) / **20** pela intenção do painel (ver acima e DH-003EC-01(b)) — **movimento desde 003.ED: 20→21 / 19→20** (R-ESP-02 materializada, R-ESP-01 corretamente excluído por ser DEPRECATED) · vocabulário/CAS **50/79** slugs (63%), **inalterado desde 003.DK** (003.EI populou `termos:` de vocabulário de espirometria, não o campo `cas`) · índice D-ARQ **sincronizado** (`python -m scripts.medir_painel`, medido nesta tiragem) · suíte **990 passed, 6 skipped**, medida com árvore parada no commit de fechamento da fatia 1 (`ae5984c`) — MEDIDO nesta sessão, não herdado; `mypy --strict agente_medico/motor agente_medico/tests/invariantes.py` delta-zero, 34 arquivos. **Correção de rótulo herdada (003.EC):** a linha "Índice CAS" da Camada 1/2 abaixo citava **21/45** (medição manual 003.AI, 23/06/2026) até a tiragem 003.EC corrigir para o número medido do instrumento, 50/79 — mantido, sem mudança nesta tiragem.
...
- **Numerador (instrumento):** 21 IDs casados por `git grep` de `R-[A-Z]+-[0-9]+` em `regras.yaml` ∪ `motor/*.py` — **21/42 (50%)**.
- **Numerador (intenção do painel):** 20 IDs com footprint **executável** — R-TEMP-01 sai (citação em prosa de `base_normativa`, sem `quando`/`emite`, não roda; ver DH-003EC-01(b)) — **20/42 (~48%)**.

| Superfície | n | IDs |
|---|---|---|
| `regras.yaml` (dado) | 11 | AUD-01/02, BIO-04, CLI-01, CLI-02, ESP-02, FDS-04, RX-01/02, VIB-01/02 |
...
| **União (instrumento)** | **21** | 50% de 42 |
| **União (intenção do painel, exclui TEMP-01)** | **20** | ~48% de 42 |

**Caveat de instrumento (não superinterpretar):** num loader data-driven, regra em `regras.yaml` é funcional **sem** string no motor. Os 21 (ou 20) são **piso de rastreabilidade**, não teto de função. E, desde 003.EC, o instrumento também não distingue `id:` de citação em prosa dentro do mesmo arquivo — ver DH-003EC-01(b).
...
- **21 IDs** sem ocorrência em `agente_medico/` — só-escritas (42 ativos − 21 no instrumento; recalculado nesta tiragem — o valor anterior, 23, não fechava aritmeticamente contra 42−20)
