...
**Tiragem corrente:** 003.EB · 25/07/2026
**Baseline:** branch `feat/003eb-rodada-offline` sobre main `495e634` · 963 passed, 6 skipped · PROTOCOLO v67 · DECISOES v150
...
|---|---|---|
| Quanto da regra clínica está no código? | **18 de 42** ativas (~43%), medido por `scripts/medir_painel.py` (003.DE) | Correção da reconciliação 003.DE: R-ECG-01/R-OP-01/R-VIS-01 saem do numerador (vivem SÓ em `agentes.yaml` campo `protocolos_especiais` — superfície de dado SEM consumidor em runtime, zero grep em `motor/`; decisão da tiragem: não conta enquanto não houver consumo executável); R-CLI-02 (003.CU) e R-GHE-01 (003.DD) entram. Pausa da frente clínica segue vigente — nenhuma regra nova foi puxada pela clínica, o movimento veio de reconciliação de instrumento e da porta de entrada |
...
**Instrumento oficial nesta tiragem (disco, branch `feat/003eb-rodada-offline` sobre `main 495e634`):** regras **18/42** ativas (43%, sem drift desde 003.DK/DE) · vocabulário/CAS **50/79** slugs (63%, inalterado) · suíte **963 passed, 6 skipped** (003.EA: +6 exato, D-ARQ-65 fatia 2; 003.EB não move a suíte — harness `scripts/` sem cobertura por precedente). **Delta registrado, não corrigido:** a linha "Índice CAS" da Camada 1/2 abaixo ainda cita **21/45** (medição manual 003.AI, 23/06/2026) — 3 sessões (003.DK/DQ/DX) de defasagem entre o instrumento oficial e a prosa longa. O refresh dessa prosa segue deferido (ver Nota de escopo desta tiragem); o número correto de disco é o do instrumento, acima.
...
- **Numerador:** 18 IDs com footprint executável (`regras.yaml` ∪ `motor/*.py`).

| Superfície | n | IDs |
|---|---|---|
| `regras.yaml` (dado) | 9 | AUD-01/02, BIO-04, CLI-02, FDS-04, RX-01/02, VIB-01/02 |
...
| **União (materializado)** | **18** | ~43% de 42 |

**Caveat de instrumento (não superinterpretar):** num loader data-driven, regra em `regras.yaml` é funcional **sem** string no motor. Os 18 são **piso de rastreabilidade**, não teto de função.

- **yaml-only** (dado existe, zero match em motor/teste): CLI-02, FDS-04, RX-02 — 3 IDs, candidatas a backfill de teste, **não** mortas
